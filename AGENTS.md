...
The VM snapshot already has the toolchain set up (Node, pnpm, Rust + `wasm32-unknown-unknown`). The startup update script only runs `pnpm install`. Standard build/test/lint/dev commands are in the "Development Commands" section above. Non-obvious caveats specific to this environment:

- **Node version matters for `zstd`.** The repo pins `v22.18.0` in `.node-version`, and `@workflow/core` serialization uses `node:zlib` zstd, which only exists on Node ≥ 22.15. The base image's default `node` (`/exec-daemon/node`) is older, so `~/.bashrc` prepends the pinned nvm Node (`v22.18.0`) to `PATH` (with a corepack `pnpm` shim). If `packages/core` compression tests fail with `expected 'gzip' to be 'zstd'`, you are running on the wrong Node — check `node --version`.
- **`pnpm build` compiles the Rust SWC plugin** (`@workflow/swc-plugin`) to WASM via Cargo. This needs Rust stable (≥ 1.87, for edition 2024) and the `wasm32-unknown-unknown` target. Both are preinstalled in the snapshot.
- **`pnpm test` runs the SWC cargo tests through turbo.** `RUSTUP_HOME`/`CARGO_HOME` point at `/usr/local/{rustup,cargo}`, but turbo's strict env mode only forwards them for the `build` task, not `test`. `~/.rustup` is therefore symlinked to `/usr/local/rustup` so the rustup proxy still finds the default toolchain; without it, `cargo test` fails with "rustup could not choose a version of cargo to run".
- **`@workflow/world-postgres` tests require Docker** (Testcontainers). Docker is not installed here, so those tests fail/skip with "Could not find a working container runtime strategy". This package is optional — the rest of `pnpm test` (all other packages) passes without it. The default/local filesystem world needs no external services.
- **`pnpm lint` (`biome check`) is not clean and is not a CI gate.** The CI "Lint" workflow only validates docs links (`bun ./scripts/lint.ts`, run inside `docs/`); formatting is enforced via the `lint-staged` pre-commit hook. Don't treat the large `biome check` error count as something to fix wholesale.
- **Local end-to-end runs** use the `nextjs-turbopack` workbench on the local world (no DB/Vercel needed): `cd workbench/nextjs-turbopack && WORKFLOW_PUBLIC_MANIFEST=1 pnpm dev` (port 3000), then trigger workflows from the home UI or run `DEPLOYMENT_URL=http://localhost:3000 APP_NAME=nextjs-turbopack pnpm vitest run packages/core/e2e/e2e.test.ts`.
