/**
 * Source snippets for the Debounce registry entry.
 *
 * Distributed debounce-by-key: collapse a burst of events into a single
 * action that fires after a quiet period. A short-lived coordination
 * workflow per key collects events; each new event resets the timer by
 * spawning a fresh timer child carrying a sequence number — stale timers
 * are ignored. When the timer finally wins, the action fires once with the
 * latest payload and the run exits.
 */

const DEBOUNCE_BODY = `import { defineHook, sleep } from "workflow";
...
export const debounceWorkflowSource = DEBOUNCE_BODY;

export const debounceWorkflowInstallSource = `/**
