/**
 * Source snippets for the Rate Limiter registry entry.
 *
 * Proactive, distributed rate limiter: bound YOUR outbound request rate to
 * an API across all workflow runs and machines. A coordination workflow
 * grants request slots spaced at least `intervalMs` apart (a "smooth"
 * limiter — e.g. 100ms interval = max 10 requests/second cluster-wide).
 *
 * Complements the Handling Rate Limits pattern, which reacts to 429s with
 * RetryableError; this one prevents you from hitting the limit at all.
 */

const RATE_LIMITER_BODY = `import { createHook, defineHook, sleep } from "workflow";
...
export const rateLimiterWorkflowSource = RATE_LIMITER_BODY;

export const rateLimiterWorkflowInstallSource = `/**
