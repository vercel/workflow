/**
 * Source snippets for the Batch Aggregator registry entry.
 *
 * The inverse of fan-out batching: accumulate individually-arriving events
 * into a buffer, then flush once when the buffer hits N items or T elapses
 * — whichever comes first. A short-lived coordination workflow per stream
 * key holds the buffer; the flush deadline arrives as a timer message.
 */

const BATCH_AGGREGATOR_BODY = `import { defineHook, sleep } from "workflow";
...
export const batchAggregatorWorkflowSource = BATCH_AGGREGATOR_BODY;

export const batchAggregatorWorkflowInstallSource = `/**
