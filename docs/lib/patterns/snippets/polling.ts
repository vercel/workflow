/**
 * Source snippets for the Polling registry entry.
 *
 * Wait-for-condition: poll an external system with exponential backoff
 * until a condition holds (or a deadline passes). Durable sleeps mean a
 * day-long poll costs nothing while waiting and survives restarts.
 */

const POLLING_BODY = `import { sleep } from "workflow";
...
async function checkCondition(
  ...
  const res = await fetch(\`https://api.example.com/status/\${target}\`);
  if (!res.ok) {
...
export const pollingWorkflowSource = POLLING_BODY;

export const pollingWorkflowInstallSource = `/**
