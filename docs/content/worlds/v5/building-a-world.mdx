...
Note the version a World stamps is gated: `mintedSpecVersion()` answers 7 only where [`WORKFLOW_SEALED_LOG`](/docs/configuration/runtime-tuning#workflow_sealed_log) is enabled, and the slot-identity floor otherwise. Declare `mintedSpecVersion()` rather than a literal so your World moves with the fleet, and note that a run created at spec 7 may be read by a runtime other than the one that created it — the reader has to understand `noop` before anything stamps 7 in that environment.
