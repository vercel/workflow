...
A rejection may also report the comparison it was made on, as `recordedAtOrBelow` and `stateUpdatedAt` on the same `details` — how many events you had recorded at or below the snapshot's watermark, and the watermark they were counted against. Send both or neither. The runtime uses them to check that a reloaded event log actually reached that count before it spends another replay on it, so a reload that keeps coming up short escalates immediately instead of repeating. Omitting them is always valid: the runtime then treats each reload as unchecked rather than as unsatisfied.
