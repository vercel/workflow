...
- Resolve no later than roughly `timeoutMs` with the latest snapshot, whatever its status. **A timeout is a normal return, never an error**: a run that is still running is a legitimate answer, and the runtime asks again.
...
Omitting the method is a supported choice: a store with no change notification, or a deterministic simulator like `world-sim` where a real wait would stall a virtual clock, leaves it off and the runtime keeps interval-polling `get`. Nothing else degrades; there is no capability to declare.
...
The runtime also sends an internal `HealthCheckPayload` through the same workflow queue.
