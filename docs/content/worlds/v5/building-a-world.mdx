...
The optional `capabilities` object advertises additional behavior, and every capability **fails closed**. A missing member means "unsupported," and the runtime keeps its conservative behavior. Set `hookRetention.active` to `true` only when the World implements hook token retention. Set `queueRedeliversUnacked.active` to `true` only when your queue redelivers a message whose consumer died before acknowledging it, on its own and within the inline ownership lease (`WORKFLOW_INLINE_OWNERSHIP_LEASE_SECONDS`, 860 seconds by default). The runtime then stops re-sending the step message for a step that a queue delivery is already running (its latest `step_started` is bare and has no terminal event) and arms one delayed backstop wake per replay pass for the run instead, relying on your redelivery to recover the step if that delivery dies. It only does so for runs at spec version 6 or later, whose runtime stamps every inline start; on older runs a bare start is not proof of a queue delivery, so they keep the immediate re-send. Leave it unset for an in-process queue, or when a dead consumer's message can stay locked for longer than the lease. Set `maxConcurrency` only when the World's queue supports `maxConcurrency`-limited consumption, which `WORKFLOW_SEQUENTIAL_REPLAYS=1` uses.
