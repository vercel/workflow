...
The required `specVersion` declares which Workflow protocol spec version the World implements. The runtime accepts the range from its own `SPEC_VERSION_CURRENT` up to `SPEC_VERSION_MAX_SUPPORTED` and refuses to create or replay runs outside it, so import the constant from `@workflow/world` rather than hardcoding a number, and re-check it on SDK upgrades.
...
`eventCount` supersedes the `stateUpdatedAt` / `stateEventCount` / `stateCursor` triple below. The triple approximates a position with a timestamp watermark plus a count of events at or below it, which a complete-but-stale snapshot passes: every event the writer holds is at or below its own watermark, so the count matches and no fence fires. A dense position has no such blind spot.

### Optional: Rejecting a Stale Write

**No World in this repository does this, and a v5 World should not need to.** A replay whose log is out of date is not dangerous, for three reasons that hold together once positions are allocated at the commit:

- A reader's log is a **prefix** of the run's log rather than a prefix with a hole in it. Nothing lands behind a position it has already passed.
- Replay is **deterministic on a prefix**. The same prefix always yields the same decisions, so a shorter log means a run that has not caught up, never a run that will decide differently.
- Every write **reports what it missed**. It names the position it was derived from, and you return the events it was pushed past. The replay merges those and continues, correcting itself on the write rather than on a read.

That is why the `preconditionGuard` capability no longer exists: the need for a guard was removed rather than made universal. The rest of this section is for the one case that still wants it — a store that allocates positions somewhere other than the commit, and therefore cannot report a skipped span reliably.

`events.create()` params may carry a description of the snapshot the caller replayed from:
