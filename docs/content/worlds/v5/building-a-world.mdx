...
The optional `capabilities` object advertises additional behavior, and every capability **fails closed**: a missing member means "unsupported", and the runtime keeps its conservative behavior. Set `hookRetention.active` to `true` only when the World implements Hook token retention. Set `slotEventIds` when the World allocates [slot-numbered event IDs](#event-id-allocation), which a v5 World is required to do. Set `preconditionGuard` only when the World actually rejects event creations carrying a stale `stateUpdatedAt` snapshot with a `PreconditionFailedError` — advertising it without the fence enables runtime optimizations with nothing behind them. Set `maxConcurrency` only when the World's queue supports `maxConcurrency`-limited consumption (used by `WORKFLOW_SEQUENTIAL_REPLAYS=1`).
