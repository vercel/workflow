...
When you deploy to Vercel, workflows automatically use the Vercel World with no setup required.
...
Vercel automatically performs these tasks:

- Selects the Vercel World backend
- Configures authentication using OpenID Connect (OIDC) tokens
- Provisions storage and queuing infrastructure
- Isolates data per environment (production, preview, and development)
...
The Vercel World runs in every [Vercel Functions region](https://vercel.com/docs/regions). Each workflow run is pinned to a single region at creation time. Its stored state, queue dispatch, and streams are served from that region, with no cross-region round trips on the hot path. When your application is deployed in the run's region, as described in [automatic region pinning](#automatic-region-pinning), step execution is also region-local.
...
- Deploy to multiple regions for a globally distributed audience, and each run is pinned to the region that served the user who triggered it, so workflow data and streaming stay close to that user.

### Explicit region selection

To pin a specific run to a different region, pass the `region` option to [`start()`](/docs/api-reference/workflow-api/start):
...
<Callout type="warn">
  ...
  application is deployed to. For execution to happen in the
...
The Workflow CLI commands open the relevant page in the Vercel dashboard:
...
The Workflow CLI automatically retrieves authentication from the Vercel CLI (`vercel login`) and infers project and team IDs from your local Vercel project linking.
...
In a Vercel deployment, you do not configure the Vercel World yourself. The platform injects everything the runtime needs, including `VERCEL_DEPLOYMENT_ID`, `VERCEL_PROJECT_ID`, per-request OpenID Connect (OIDC) tokens, and `VERCEL_DEPLOYMENT_KEY` for encryption.

Do not set those platform-provided values yourself.

Most users never need to set the `WORKFLOW_VERCEL_*` variables below. They provide overrides for tools running outside Vercel, such as your laptop or continuous integration (CI), when those tools need to inspect or test a remote Vercel Workflow project and cannot infer the project, team, token, or target environment automatically.
...
The Vercel environment to target. Valid options are `production` and `preview`. Default: `production`.

### `WORKFLOW_VERCEL_AUTH_TOKEN`

Vercel API authentication token. This value is secret, so keep it in your environment instead of in code. Falls back to `VERCEL_TOKEN`, then to your Vercel CLI login.
...
| `workflow.event.type` | the event type, for example `step_started` | same |
...
- Unauthorized requests cannot reach workflow or step execution.
- Only messages delivered through Vercel Queues can trigger execution.
- Handlers receive only a message ID that must be retrieved from Vercel's backend, preventing custom payloads.

<Callout>
  This configuration is managed entirely by the Workflow SDK build step. You should not need to write this yourself. If you are writing a custom integration, see the [Security section of Framework Integrations](/docs/how-it-works/framework-integrations#security) for more details.
</Callout>
...
- **Queuing**: Steps are distributed across serverless functions through [Vercel Queues](https://vercel.com/docs/queues) with automatic retries and [consumer function security](#consumer-function-security).
