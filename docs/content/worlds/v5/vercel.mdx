...
By default, how long a finished run's data is kept is decided by your team's plan. [`start({ retention })`](/docs/api-reference/workflow-api/start#setting-data-retention) states a per-run preference:

```typescript
const run = await start(myWorkflow, [input], { retention: "none" });
```

- `"default"` (or omitting the option) applies your plan's retention period.
- `"none"` deletes user data — step and event payloads, and stream chunks — as soon as the run completes or fails. The run, step, and event records themselves are kept for your plan's default period, so a finished run stays visible in observability with its payloads gone.
- Any other value is not supported and is ignored: the run keeps your plan's default retention.
...
- **Custom retention values** - [`start({ retention })`](/docs/api-reference/workflow-api/start#setting-data-retention) supports `"default"` and `"none"`; any other value falls back to your plan's default retention.
