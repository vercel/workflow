...
For an affinity-enabled workflow endpoint, set `WORKFLOW_VERCEL_INVOKE_URL` to
its full URL on producers and receivers, or configure the World directly:
...
const world = createWorld({
  invoke: {
    endpoint: "https://example.vercel.app/.well-known/workflow/v1/invoke",
  },
});
```
...
entry point using their workflow execution handler. Reusing handler code does
not guarantee that the HTTP and queue routes share a running process.

The default is disabled. The endpoint must already support function affinity and
deployment selection; setting this URL does not provision those platform features.
Requests use `x-vercel-affinity-id: runId` and `x-deployment-id` for the run's
pinned deployment. The deployment header follows [Skew Protection](https://vercel.com/docs/skew-protection)
and its retention/enablement constraints. An async endpoint resolver receives
`{ runId, deploymentId, region }` for custom routing. The region identifies storage
and queue placement, and may differ from compute placement.

Supported hook inputs travel through a synchronous, binary-safe HTTP call rather
than VQS. The receiving `createQueueHandler` has a direct branch: authenticate,
enqueue in a private per-run in-memory mailbox, await actual SDK handler
processing, and return its value or typed error. A separate local execution lane
supports cold arrival and hooks sent while inline steps wait. Continuations use
the Vercel request lifetime (`waitUntil`); normal step/timer scheduling remains.
Ordinary orchestration deliveries do not require affinity headers. Missing or
different affinity headers are logged and execution continues. Direct hook calls
send the run's affinity key; received selectors are recorded for comparison rather
than used to reject execution. Workload authentication and actual pinned-deployment
checks still apply. Routing records include invocation and request IDs, process
identity, region, observed headers, and elapsed time without tokens or payloads.

Ingress requires a verified workload OIDC token for the same project/environment.
The client obtains one automatically in a Vercel request, or through
`invoke.getToken()`. Ordinary API tokens are not accepted for this direct ingress.
Hook writes require fresh backend dedup attestation. A response means the input
was processed, not that the whole workflow finished.

The response deadline defaults to 30 seconds (`timeoutMs`: 1–120,000 ms), and
encoded request/response bodies are limited to 1 MiB. Mailboxes admit at most 32
pending inputs per run, 128 across a handler, and 64 live run states; overload is
rejected with 429. Timeout or instance loss is an unknown outcome. Retry the same
logical input with its original identity and payload; the adapter neither retries
POST automatically nor falls back to a producer write or queue delivery.

This opt-in increment covers hooks and mailbox delivery. Other journal mutation
paths and existing consistency protections remain; it does not yet establish
complete single-writer enforcement or stale-writer fencing. Validate deployment
pinning and real affinity placement before enabling it on a deployment.
Acceptance confirms input processing, not eventual continuation success. A later
continuation failure is logged and requires existing runtime recovery or another
execution request; this transport adds no durable recovery wake.

### Retained runner

For new same-deployment Node VM runs, `WORKFLOW_RETAINED_RUNNER=1` enables a
resident owner behind the direct invocation endpoint. The World must provide
exclusive per-run delivery. The owner replays once, validates hook inputs using
in-memory state, and serializes event persistence and state updates before
acknowledging each input. Its VM stays resident while suspended for up to
60 seconds, subject to the host deadline.

Step bodies run as asynchronous workers in the owning process; their state
changes return to the same mailbox loop. Queue wakes use the direct endpoint.
Conflicting events and persistence failures stop the owner, attempt a durable
run failure, and reject unfinished inputs. An inability to record that terminal
failure is reported explicitly. This mode does not retry failed event writes
or reconcile conflicting history.

Node diagnostics channels `workflow.runner` and `workflow.execution` expose
owner turns, persistence, step execution, failure recording and VM mode without
including workflow payloads. Existing pinned runs keep their previous behavior.

### `WORKFLOW_OWNER_JOURNAL` (experimental)

Set `WORKFLOW_OWNER_JOURNAL=1` only with retained-owner execution and a compatible
eventsync backend (`WORKFLOW_EVENTS_TRANSPORT=eventsync`). Newly created runs
persist `executionContext.ownerJournalVersion: 1`; older runs keep their existing
mode. This requires exclusive ownership across runner handoff and unfinished
storage requests; the transport itself does not acquire that ownership.

The owner validates run-local transitions in memory and commits contiguous event
prefixes sequentially. All canonical events remain intact. The backend derives
Step state from them, commits one event with an ordinary Put, and commits a larger
prefix atomically. Flush-through barriers precede input acknowledgement and user
step execution. Shared-resource operations still use their native independent
path. Terminal failure stays on the same owner writer; an uncertain or broken
writer reports `terminalPersisted=false` rather than starting a competing write.
