...
The Vercel World caps a single run's [event log](/docs/how-it-works/event-sourcing) at **25,000 events**. The limit is owned by the service and reported to the runtime on each run — [`WORKFLOW_MAX_EVENTS`](/docs/configuration/runtime-tuning#workflow_max_events) does not raise it. A run that reaches the ceiling fails with `MAX_EVENTS_EXCEEDED` and cannot be continued.

Events are not steps. A step that succeeds on the first attempt records three of them (`step_created`, `step_started`, `step_completed`); retries, hooks, sleeps, and webhooks each add their own. So a run made entirely of successful steps reaches the ceiling at roughly 8,000 steps, not 25,000 — and a retry-heavy one well before that.

Design for the ceiling as a backstop rather than a budget: replay work grows with the log, so a run that is close to 25,000 events is already paying for its size in per-step latency. Split workloads whose step count scales with their input into [child workflows](/cookbook/advanced/child-workflows). See [How much work fits in one run](/docs/foundations/workflows-and-steps#how-much-work-fits-in-one-run) for the fan-out and batching guidance that goes with it.
