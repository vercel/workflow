...
The Vercel World caps a single run's [event log](/docs/how-it-works/event-sourcing); see [Workflow run limits](https://vercel.com/docs/workflows/pricing#workflow-run-limits) for the current ceiling. A run that reaches it fails with `MAX_EVENTS_EXCEEDED` and cannot be continued.

Treat the cap as a backstop, not a budget. If you expect a run to accumulate tens of thousands of events, split the work into [child workflows](/cookbook/advanced/child-workflows) so each run's log stays small.
