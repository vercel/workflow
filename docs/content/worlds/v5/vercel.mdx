...
Serializing a run's orchestrator invocations also lets the first-delivery fast path (`WORKFLOW_TURBO`) keep optimistic inline step start active after the run creates a hook or wait; without it, the fast path stops forcing optimistic start at that point because hook and wait resume invocations could otherwise run concurrently.
