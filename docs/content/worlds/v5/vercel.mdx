...
**At most one orchestrator (flow) invocation runs at a time per workflow run.** This is on by default; set `WORKFLOW_SEQUENTIAL_REPLAYS=0` to turn it off and let the runtime rely on idempotency and the event log to tolerate concurrent flow invocations of the same run.
...
Because a run's wake-ups (`sleep()` continuations, hook resumes) share that single slot, a run with an armed timer or an open hook stops executing step bodies inside the orchestrator invocation: each step is dispatched to its own topic instead, so the wake-up can be delivered while the step runs. That costs one queue hop per step for those runs, and is what keeps `Promise.race()` between a step and a `sleep()` resolvable. Runs that are a plain sequence of steps are unaffected and keep executing inline.
...
Because it routes each run's flow invocations through a dedicated `maxConcurrency: 1` queue topic, this might lead to higher queue performance overhead.
