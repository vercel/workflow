...
Treat both caps as backstops, not budgets. Replay cost grows with the log, so a run slows down well before it fails: the pricing page recommends [child workflows](/cookbook/advanced/child-workflows) once a run would grow past a few thousand events, which is an order of magnitude below the ceiling. Size for that recommendation rather than for the cap.
