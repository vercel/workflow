...
The Vercel World caps a single run's [event log](/docs/how-it-works/event-sourcing), see [limits page](https://vercel.com/docs/workflows/pricing#workflow-run-limits). A run that reaches the ceiling fails with `MAX_EVENTS_EXCEEDED` and cannot be continued.


Design for the ceiling as a backstop rather than a budget: replay work grows with the log, so a run with tens of thousands of events is already paying for its size in per-step latency, even before the limit. Split workloads whose step count scales with their input into [child workflows](/cookbook/advanced/child-workflows). See [How much work fits in one run](/docs/foundations/workflows-and-steps#how-much-work-fits-in-one-run) for the fan-out and batching guidance that goes with it.
