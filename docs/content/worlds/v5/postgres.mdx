...
Both handler results and handler-thrown errors are stored as versioned invocation
outcomes. The caller receives the value or a restored Workflow error (including
fields such as status, code, run ID, and expiry time). A stored handler error is
