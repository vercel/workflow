...
<Callout type="info">
Keep `workflow` and `@workflow/world-postgres` on the same version. If your app
uses a prerelease Workflow version, install the matching prerelease Postgres
World package (for example `npm install @workflow/world-postgres@5.1.0-beta.1`
alongside `workflow@5.1.0-beta.1`). Mismatched versions fail before starting a
run with an error that says the runtime requires a World with a matching spec
version.
</Callout>
