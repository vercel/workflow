...
Only deliveries verified against an active Graphile executor job on the run's
named queue start mailbox service. Updated workers/HTTP handlers redirect
legacy, unmarked orchestration deliveries into that queue before acknowledging
them. Step jobs and health checks never consume the mailbox. The entry check
does not fence a handler that continues after its job is reclaimed.

The World invokes the SDK handler with `invoke: true`, a stable `requestId`, and
the input. The SDK inspects and persists the hook event, then returns a result;
the World stores that result for the caller. No World mailbox or response callback
is exposed to the runtime.

Handler results and terminal handler errors are stored as versioned invocation
outcomes. The caller receives the value or a restored Workflow error (including
fields such as status, code, run ID, and expiry time). Transient and unknown
infrastructure errors remain pending for Graphile redelivery. A stored handler error is
distinct from failure to persist/deliver an outcome, which remains unknown.
Migration 0022 preserves the interpretation of legacy stored return values;
upgrade invocation producers and workers together.

Before acknowledging an idle executor or ending its input-intake budget, the
World joins in-flight input delivery and replays any newly committed input. A
slow hook write cannot consume its only wake without a subsequent replay.

Inputs/results are stored in a Postgres table. A shared, lazily opened
`LISTEN/NOTIFY` connection wakes the active input feed and waiting callers when
their rows change. Notifications carry identifiers only; waiters reread the
table. A 1-second fallback handles missed signals, and listener failures permit
reconnection with a 1-second backoff. The listener requires a session-capable
database connection; polling preserves progress if LISTEN is unavailable.
Every invoke still schedules a Graphile wake.
Listener degradation/restoration is logged once per state transition without
printing payloads or connection details.

The default invoke response timeout is 30 seconds, and the encoded input/result limit is
1 MiB each. A timeout does not undo processing. Event persistence precedes
response persistence; durable resume identities deduplicate the event if response
storage fails. `$retention: 0` also purges invocation data and prevents late writes
from restoring it; callers whose result has expired receive a 410 error with code
`INVOCATION_DATA_EXPIRED`. General result cleanup, stale-executor fencing and
routing across incompatible code versions remain limitations.
