...
- `manifest.json` responds with `404` unless `WORKFLOW_PUBLIC_MANIFEST=1`. Leave it unset outside of testing, because the manifest lists your workflow and step names.
...
- `/.well-known/workflow/` restricted so untrusted clients cannot reach it (see [Security](#security))
