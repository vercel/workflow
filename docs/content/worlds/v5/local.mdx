...
Whether queue deliveries go out on the runtime's own `fetch()` connection pool instead of the one this World builds. Default: enabled. Set to `0` to keep this World's own pool. While enabled, `WORKFLOW_LOCAL_HEADERS_TIMEOUT_MS` and `WORKFLOW_LOCAL_BODY_TIMEOUT_MS` are inert, since the bounds they set live on the pool this World no longer owns.
