...
`await run.returnValue` asks the World to wait for the run to finish. When the run and the caller share a process — the usual dev-server case — an in-process signal resolves the wait as soon as the run ends, and this interval never comes into play. It is the backstop that covers everything the signal cannot see, chiefly a second process awaiting a run over the same data directory, so it is set far below the SDK's own polling interval.
