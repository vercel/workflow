...
A World declares the protocol version it speaks on `specVersion`. In v4 the runtime required that number to equal its own current version exactly. In v5 it accepts a range, from the current version up to the highest version the SDK can read, so a World may opt into a version above the default without the runtime refusing it.

The default for v5 is `SPEC_VERSION_CURRENT`, which is `5`. Declare that unless you implement the newer event ID scheme described below, in which case declare `SPEC_VERSION_SUPPORTS_SLOT_IDENTITY`. Both constants are exported from `@workflow/world`; import them rather than hardcoding a number, so a later bump doesn't silently strand your implementation.
...
This is the largest change for a World implementation, and it is optional.

In v4 an event ID was a ULID your World minted however it liked. In v5 a World may instead allocate **dense slot IDs**: `evnt_` followed by the event's 1-based position in that run's log, zero-padded to 26 characters, so a run's first event is `evnt_00000000000000000000000001`. Format one with `slotToEventId()` from `@workflow/world`, declare `capabilities.slotEventIds`, and declare `SPEC_VERSION_SUPPORTS_SLOT_IDENTITY`.

The point is what a reader can conclude from a log it just fetched. Positions are dense, so the length of a log is enough to tell a complete one from a truncated one, without a second read to confirm. That is what makes an event log safe to replay across, and it is why the two invariants below are not negotiable if you adopt the scheme:
...
Callers help you place the write: `events.create()` params carry `eventCount`, the number of events the writer held in the log it replayed from, so the position it expects is `eventCount + 1`. When that position is already taken, **do not reject the write**. Advance to the next free position, commit there, and return the events occupying the positions you skipped on the success response. The writer merges them and replays once. A stale count is the normal case for a parallel fan-out, so rejecting it would serialize writes the runtime deliberately issues concurrently.

For a World that allocates positions, `eventCount` supersedes the `stateUpdatedAt` / `stateEventCount` / `stateCursor` triple used by the [precondition guard](/worlds/building-a-world#optional-the-event-creation-precondition-guard). The triple approximates a position with a timestamp watermark plus a count of events at or below it, which a complete-but-stale snapshot passes: every event the writer holds is at or below its own watermark, so the count matches and no fence fires. A dense position has no such blind spot.

Nothing forces this on you. A World that keeps minting ULIDs declares spec version 5, leaves `slotEventIds` unset, and keeps working. See [Building a World](/worlds/building-a-world) for the full allocation rules alongside the rest of the storage contract.
...
| Member | What it buys |
| --- | --- |
| `capabilities` | Advertises `hookRetention.active`, `preconditionGuard`, `hookResumeDedup`, `deploymentAffinity`, `maxConcurrency`, and `slotEventIds`. See the contract note above about failing closed. |
