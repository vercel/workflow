...
Two consequences are specific to an upgrade. `eventCount` supersedes the `stateUpdatedAt` / `stateEventCount` / `stateCursor` triple used by the [precondition guard](/worlds/building-a-world#optional-the-event-creation-precondition-guard), which a complete-but-stale snapshot passes and a dense position does not. And runs already in your store are unaffected: a run's scheme is pinned by the run rather than by the capability flag, and it is readable off the shape of its own first event ID, so a World that switches slots on keeps replaying its existing ULID-numbered runs unchanged. `capabilities.slotEventIds` states what *new* runs get.
