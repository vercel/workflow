...
Whether pending and running runs found in the data directory are re-enqueued when the world starts. Set to `0` or `false` to skip recovery and leave stale runs untouched. Default: `true`. Requires `workflow` 5.x.

### `WORKFLOW_STREAM_FLUSH_INTERVAL_MS`

Flush interval, in milliseconds, for buffered stream writes. Default: `10`. Requires `workflow` 5.x.
...
<Callout type="info">
  On the 4.x release line, the factory function is `createLocalWorld()` —
  `createWorld()` is available starting with `workflow` 5.x.
</Callout>
