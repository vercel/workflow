...
`VERCEL_DEPLOYMENT_ID` is available at both build time and runtime, and Workflow reads it in both. The build uses it to decide whether to emit Vercel Function output, including the queue-triggered flow handler, or local development output. The runtime uses it to select the World.
...
<Callout type="warn">
  ...
  output and the runtime selects the [Local World](/worlds/local), which stores
  workflow state on the filesystem. A deployment's filesystem is read-only, so
  every run fails before its first step, and no runs appear in the Vercel
  dashboard because none were ever created in the Vercel World.
</Callout>

To confirm this is what you are hitting, log `process.env.VERCEL_DEPLOYMENT_ID` from a deployed function. If it is undefined (as is `VERCEL`), the setting is off.

Setting `WORKFLOW_TARGET_WORLD=vercel` is not a workaround. The Vercel World needs the deployment ID itself, to address queue messages to the deployment that created each run and to derive that deployment's encryption keys. Enable the checkbox and redeploy.
...
The Vercel World requires no configuration when deployed to Vercel, beyond leaving [system environment variables](#system-environment-variables) enabled for the project so that `VERCEL_DEPLOYMENT_ID` reaches your build and your functions. For advanced use cases, you can override settings programmatically via `createVercelWorld()`.
