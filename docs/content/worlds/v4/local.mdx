...
<Callout type="warn">
The dev cancellation runs only when the server process **restarts**. If you edit a workflow while a run is in flight and the dev server hot-reloads the module without restarting (HMR / Fast Refresh), that run can replay against the changed code and hit a [replay divergence](/v5/docs/errors/replay-divergence). Restart the dev server (which cancels the stale run) or cancel the run manually if that happens.
</Callout>
