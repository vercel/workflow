...
2. Find the affected runs in [observability](/docs/observability/overview) or with the CLI.
...
In the [observability UI](/docs/observability/overview), use **Rerun on latest** to enqueue the workflow again with the same inputs against the latest deployment.
