...
The `--workflowName` filter expects the generated workflow ID, not only the exported function's short name. Use the `workflowName` value from `workflow inspect runs`.
...
Anything that is serializable by Workflow SDK can be passed from one run to the next as an argument. That includes plain state objects, `ReadableStream`, `WritableStream`, and other supported serialized values.
...
export async function exportReport(
  ...
  // Register the stream as early as possible. Continuation runs receive
  // this same stream as an argument and keep writing to it.
  const stream = progress ?? getWritable<string>();
