...
2. Find the affected runs in [observability](/docs/observability) or with the Workflow CLI.
...
Model those as a sequence of runs. Each run does a bounded piece of work, then starts the next run on the latest deployment and exits. This is similar to `continueAsNew` in other durable execution systems, but Workflow SDK uses [explicit recursion through `start()`](/patterns/workflow-composition).
