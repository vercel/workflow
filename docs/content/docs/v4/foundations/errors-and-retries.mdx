...
description: Customize retry behavior with FatalError and RetryableError for controlled error handling.
type: conceptual
...
| `MAX_EVENTS_EXCEEDED` | The run reached the World's per-run event ceiling. Split unbounded loops into child workflows before reaching the limit |
...
| `CORRUPTED_EVENT_LOG` | The event log cannot be replayed because it contains orphaned or mismatched events, a gap, or an unreadable stored payload. If you see this, please [file an issue](https://github.com/vercel/workflow/issues) |
