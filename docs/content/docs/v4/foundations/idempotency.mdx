...
export async function POST(request: Request) {
  ...
  if (result.status === "duplicate") { // [!code highlight]
    // This request's run lost the race; route the caller // [!code highlight]
    // to the active owner it reported. // [!code highlight]
...
<Callout type="warn">
This avoids creating a new run only after the first run has registered its hook. Because `start()` returns before the run body executes and calls `createHook()`, two concurrent requests can both observe "no hook yet" and each call `start()`. The race is resolved inside the workflow body, where the second run observes `getConflict` resolving with the active owner and returns a duplicate status. A native API for atomically starting a run and registering a hook is in the works. Until then, model recovery inside the workflow by checking `hook.getConflict`.
</Callout>
...
{/* @skip-typecheck - strategy fragment, see processOrder and the step helpers above for full context */}
```typescript lineNumbers
const conflict = await request.getConflict;
if (conflict) {
  return (await getOwnerResult(conflict.runId)) as OrderResult; // [!code highlight]
}
```
