...
export async function POST(request: Request) {
  ...
  if (result.status === "duplicate") { // [!code highlight]
    // This request's run lost the race; the resume above // [!code highlight]
    // reached the active owner. // [!code highlight]
    return Response.json({ runId: resumed.runId, reused: true }); // [!code highlight]
  } // [!code highlight]
...
<Callout type="warn">
This avoids creating a new run only after the first run has registered its hook. Because `start()` returns before the run body executes and calls `createHook()`, two concurrent requests can both observe "no hook yet" and each call `start()`. The race is resolved inside the workflow body, where the second run observes `hasConflict` resolving with `true` and returns a duplicate status. A native API for atomically starting a run and registering a hook is in the works. Until then, model recovery inside the workflow by checking `hook.hasConflict`.
</Callout>
