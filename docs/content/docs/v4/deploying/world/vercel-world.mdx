...
On Vercel, workflow runs are pegged to the deployment that started them. For the full model, including rerunning on latest and explicit upgrade boundaries, see [Versioning](/docs/foundations/versioning). This means:
