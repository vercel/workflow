...
Where to call this depends on your framework (Next.js `instrumentation.ts`, SvelteKit `init`, a Nitro server plugin, etc.) — see [Recovering in-flight runs](/docs/deploying/recovering-in-flight-runs).
