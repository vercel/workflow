...
- [`sleep()`](/docs/api-reference/workflow/sleep): Provides a durable wait that survives restarts at zero compute cost.
- [`createWebhook()`](/docs/api-reference/workflow/create-webhook): Creates a webhook URL the workflow can race against.
- [`defineHook()`](/docs/api-reference/workflow/define-hook): Creates a typed hook for in-process cancellation.
- [Idempotency](/docs/foundations/idempotency): Protects side effects that may keep running after a timeout.
- [`Promise.race()`](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Promise/race): Races operations against deadlines.
