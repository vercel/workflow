...
- **Call `start()` from a step in v4**: Don't call it directly from a workflow function. Bake the wrapped workflow reference into the step, and don't pass workflow functions as step arguments.
...
- **Use chunked spawning for large batches**: Spawning 500 children in a single step can time out. Break the work into chunks of 10-50.
