...
description: Spawn child workflows from a parent and wait for completion via hook resume for batch processing, report generation, and other multi-workflow orchestration scenarios.
type: guide
...
This replaces polling `getRun().status` in a sleep loop. The parent suspends on `await hook` until the child calls `resumeHook()` — durable, efficient, and free of poll-interval latency.
...
## Alternative: polling with `getRun()`

If you need to inspect many runs that were spawned without hook tokens (e.g., legacy integrations), you can poll `getRun(runId).status` in a durable `sleep()` loop. This pattern is supported but not recommended for new code — prefer hook resume for waiting on child completion.
