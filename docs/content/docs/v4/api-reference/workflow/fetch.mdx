...
This example shows how to use `fetch` inside your workflow.
...
Call `fetch()` with a URL and optional request options, as with the standard fetch API. The workflow runtime automatically handles the response serialization.

This API provides `fetch` in a workflow, but you might want to implement your own fetch for more control over error handling and retry logic.
