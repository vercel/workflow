...
<Callout type="info">
Custom hook tokens are the recommended way to coordinate active workflow runs. Use a deterministic token from your domain, such as an order ID or conversation ID, and create the hook near the beginning of the workflow. Only one active hook can own that token: check `await hook.hasConflict` before work that depends on owning the token. If the duplicate run needs the active owner's run ID, awaiting or iterating the hook payload rejects with `HookConflictError`, and current worlds include `conflictingRunId` on that error. See [Run idempotency](/docs/foundations/idempotency#run-idempotency).
</Callout>
