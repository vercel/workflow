...
<Callout type="warn">
  `stepName` is a machine-readable identifier like `step//./src/workflows/order//processPayment`. Use [`parseStepName()`](/docs/api-reference/workflow-observability/parse-step-name) from `workflow/observability` to extract the `shortName` for display in a user interface.
</Callout>
