...
// Queue methods are called directly on world, for example, world.queue()
```
...
Gets the current deployment ID. The SDK uses the ID to route queue messages to the correct deployment.
...
Dispatches a message to a named queue. The message payload is an internal SDK type (`WorkflowInvokePayload`, `StepInvokePayload`, or `HealthCheckPayload`).
...
Creates an HTTP handler that processes messages from a queue. Use it to set up the queue consumer endpoint.
