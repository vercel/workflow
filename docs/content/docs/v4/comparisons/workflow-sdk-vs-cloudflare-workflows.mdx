...
<Callout type="info">
**Choose the Workflow SDK** when you want an open-source, portable engine that runs in your existing app (and off a single vendor), deployment-pinned versioning, and application-level E2E encryption. **Choose Cloudflare Workflows** when you're already all-in on Cloudflare and want near-zero cold starts and global edge distribution out of the box.
</Callout>
...
| **Performance** | No-penalty resume; scale-to-zero; up to 100K concurrency (Vercel) | **~0 ms isolate cold starts**; 50K concurrent instances (raised from 4,500 at GA); global Anycast (330+ cities) — strongest cold-start and edge story |
...
| **Pricing** | SDK free; pay your platform | Workers Standard: requests + CPU-time + storage (idle/sleep not billed); a per-**step** dimension is being added (500K steps/mo included, then $0.80 / 100K), effective no earlier than Aug 10, 2026 |
| **Limits** | 50 MB payloads; 2 GB/run; 10K steps | **1 MiB** step result & event payload; 1 GB state/instance; 10K steps (up to 25K) |
| **Maturity** | v4 GA / v5 beta | GA since April 2025 |
...
On portability, Cloudflare Workflows is the most locked-in of the tools in this section: the API and Durable-Object-bound state are Cloudflare-specific, so moving means a rewrite. The Workflow SDK is Apache-2.0 and its [World abstraction](/docs/deploying/building-a-world) lets you run the same code on Vercel, on Postgres, or on a backend you build.
...
---
*Compiled from public documentation as of July 2026. Verify current Cloudflare Workflows limits and pricing against [developers.cloudflare.com/workflows](https://developers.cloudflare.com/workflows/). Not based on head-to-head benchmarks.*
