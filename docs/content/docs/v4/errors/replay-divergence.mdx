...
To avoid replaying stale runs against changed code, the dev server cancels in-flight runs from the previous session when it **restarts** (see [Recovering in-flight runs](/docs/deploying/recovering-in-flight-runs#development-vs-production)). This does **not** cover hot-module reload: if your framework swaps the edited module without restarting the process (HMR / Fast Refresh), an already-running invocation keeps replaying against the new code and can diverge. If you see this after editing a workflow with a run in progress, restart the dev server (which cancels the stale run) or cancel the run manually.
