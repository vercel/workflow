...
Safe mode currently implies `WORKFLOW_PRECONDITION_GUARD=1` — the backend rejects an event written from a stale view of the run, so the runtime reloads and retries instead of recording conflicting events — and, on Vercel deployments, `WORKFLOW_SEQUENTIAL_REPLAYS=1` — orchestrator invocations are serialized per run, so two replays can never race each other.
