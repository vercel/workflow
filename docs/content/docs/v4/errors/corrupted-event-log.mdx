...
- `WORKFLOW_PRECONDITION_GUARD=1` makes the backend reject an event written from a stale view of the run, so the runtime reloads and retries instead of recording conflicting events.
