...
While a hook is active, its token is reserved and cannot be used by other workflows. If a workflow attempts to create a hook with a token that is already in use by another active hook, a `hook_conflict` event is recorded instead of `hook_created`. The event includes the token and, when available, the run ID that currently owns it. This causes the hook's promise to reject with a `HookConflictError`, which you can detect with `HookConflictError.is(error)`. See the [hook-conflict error](/docs/errors/hook-conflict) documentation for more details.
...
| `hook_conflict` | Records that hook creation failed because the token is already in use by another active hook. Contains the token and the active hook owner's run ID when available. The hook is not created, and awaiting the hook will reject with a `HookConflictError`. |
