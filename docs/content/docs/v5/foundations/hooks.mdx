...
- **Decide how long the token should stay reserved**: By default the token is reusable after the run ends. Use [`experimental_retention`](/docs/api-reference/workflow/create-hook#keep-a-token-reserved-after-the-run-ends) to block late duplicates, and do not automatically dispose that Hook.
