...
Custom tokens are available for `createHook()` with server-side `resumeHook()` only. Webhooks (`createWebhook()`) always generate their own token, drawn from the run's deterministic sequence so the URL stays stable across replays and across concurrent invocations of the same run. A generated token is not trivial to guess, but it is not a secret either: authenticate webhook requests themselves with a signature header, a shared secret, or a check inside the handler rather than relying on URL secrecy.
