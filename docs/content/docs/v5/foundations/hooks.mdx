...
`await hook.getConflict()` registers the Hook without waiting for data. It returns `null` if registration succeeds, or the run already using the token. See [Run idempotency](/docs/foundations/idempotency#run-idempotency) for handling examples.
