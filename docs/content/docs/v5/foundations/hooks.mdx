...
- **Decide when the token should expire**: By default the token expires when the run ends. Use [`experimental_expires`](/docs/api-reference/workflow/create-hook#set-when-a-hook-token-expires) to block late duplicates, and do not automatically dispose that Hook.
