...
- **Choose an explicit retention policy for idempotency claims**: By default the token is reusable after the run ends. Use [`experimental_retention`](/docs/api-reference/workflow/create-hook#retaining-a-token-claim-after-completion) when late duplicates must still conflict, and do not automatically dispose that ownership hook.
