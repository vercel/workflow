...
Race a hook against `sleep()` to stop waiting after a deadline. The race can run again in a loop: an event that arrives after an earlier race timed out still reaches the race that is currently waiting.
...
export async function reminderWorkflow() {
  ...
  for (let reminder = 0; reminder < 3; reminder++) {
    const result = await Promise.race([
      hook.then((payload) => payload),
      sleep("1d").then(() => "timeout" as const),
    ]);

    if (result !== "timeout") {
      return result.approved;
    }
    // Send a reminder, then wait again
  }
...
Every `await` on a hook that happens while no event is available waits for the same next event, so `Promise.all([hook, hook])` resolves both with one payload. To receive separate events, await them one after another or iterate with `for await...of`.
