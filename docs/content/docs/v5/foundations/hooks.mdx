...
<Callout type="info">
<LanguageContent value="ts" inline>We recommend using the `using` keyword which implements the [TC39 Explicit Resource Management](https://github.com/tc39/proposal-explicit-resource-management) proposal for automatic cleanup.</LanguageContent><LanguageContent value="py" inline>Use `async with` to dispose the hook automatically when the block exits.</LanguageContent>
</Callout>

<Callout type="info">
See the full API reference for <LanguageLink ts="/docs/api-reference/workflow/create-hook" py="/docs/api-reference/python/base-hook#wait"><LanguageText code ts="createHook()" py="BaseHook.wait()" /></LanguageLink> for all available options.
</Callout>

### Resuming a hook

To send data to a waiting workflow, use <LanguageLink ts="/docs/api-reference/workflow-api/resume-hook" py="/docs/api-reference/python/base-hook#resume"><LanguageText code ts="resumeHook()" py="BaseHook.resume()" /></LanguageLink> from an API route, server action, or any other external context:
...
async def approve(token: str, approval: Approval):
    try:
        result = await approval.resume(token)  # [!code highlight]
...
Sometimes you need to know that a hook token has been claimed, but you do not want to wait for external data yet. Await <LanguageText code ts="hook.getConflict()" py="hook.get_conflict()" /> for that:
...
Calling <LanguageText code ts="createHook()" py="BaseHook.wait()" /> on its own does not register the hook; registration is only committed when the workflow suspends. Awaiting <LanguageText code ts="hook.getConflict()" py="hook.get_conflict()" /> suspends the workflow to commit the hook registration, then resolves with <LanguageText code ts="null" py="None" /> once the hook is registered and ready to receive payloads, or with a `Run` handle for the run that owns the token (see [`HookConflictError`](/docs/errors/hook-conflict)). For `hook_conflict` events persisted by older worlds that did not record the owning run's ID, <LanguageText code ts="getConflict()" py="get_conflict()" /> rejects with `HookConflictError` instead of resolving with an incomplete handle. <LanguageContent value="ts" inline>The conflicting run's accessors are durable steps, so the workflow can inspect `await conflict.status`, wait on `await conflict.returnValue`, or cancel the owner with `await conflict.cancel()`. See [Run idempotency](/docs/foundations/idempotency#run-idempotency) for these strategies.</LanguageContent>
...
async def slack_channel_bot(channel_id: str):
    # Use channel ID in the token so Slack webhooks can find this workflow
    async with SlackMessage.wait(token=f"slack_messages:{channel_id}") as hook:
...
async def receive_slack_event(event: SlackMessage, channel_id: str):
    try:
        ...
        return PlainTextResponse("Hook not found", status_code=404)
```
...
Hooks are _reusable_. They implement `AsyncIterable`, which means you can use <LanguageText code ts="for await...of" py="async for" /> to receive multiple events over time:
...
@wf.workflow
async def data_collection_workflow():
    async with DataPoint.wait() as hook:
        values: list[int] = []
...
Each time you call <LanguageText code ts="resumeHook()" py="BaseHook.resume()" /> with the same token, the loop receives another value.

### Disposing hooks early

When a workflow ends, hooks are automatically disposed. However, you may want to release a hook token early so another workflow can use it while your workflow continues running. Use a <LanguageText ts="block scope" py="context manager" /> with <LanguageText code ts="using" py="async with" /> to control when disposal happens:
...
```python lineNumbers
hook = ChannelMessage.wait(token=f"channel:{channel_id}")
payload = await hook
hook.dispose()  # Manually release the token
```
...
The Python SDK does not currently provide the `create_webhook()` convenience API or an automatically generated public webhook URL. Use a typed hook behind an HTTP route in your application instead:
...
When using custom tokens with <LanguageText code ts="createHook()" py="BaseHook.wait()" />:

- **Make them deterministic**: Base them on data the external system can reconstruct, such as channel IDs or user IDs.
- **Use namespacing**: Prefix tokens to avoid conflicts, such as <LanguageText code ts="slack:${channelId}" py='f"slack:{channel_id}"' /> or <LanguageText code ts="github:${repoId}" py='f"github:{repo_id}"' />.
...
<LanguageText ts="Both hooks and webhooks" py="Hooks" /> support iteration, making them perfect for long-running event loops:
...
- [Serialization](/docs/foundations/serialization) - Understanding what data can be passed through hooks
- [`BaseHook` API Reference](/docs/api-reference/python/base-hook)
