...
By default, the token expires when the workflow ends. Set `experimental_expires` to keep it reserved for late duplicate requests. The Hook itself is still removed and cannot be resumed. See [`createHook()` token expiration](/docs/api-reference/workflow/create-hook#set-when-a-hook-token-expires) for examples and supported values.

`experimental_expires` stops another run from using the token until it expires. It does not cache what the first run returned. A duplicate can read `conflict.returnValue` only while the first run is still stored. If callers need that result after old runs are deleted, save it in your own database under the same key, such as the order ID.

### Conflict-handling strategies

The example above rejects the duplicate by returning the owner's `runId`. You can also handle a conflict in these ways:

If `getConflict()` returns a run, the current run does not own the token and must not do the work guarded by that token. With a later expiration time, the owner may already be finished. In that case its Hook cannot be resumed, and cancelling the finished run will not expire the token.
...
**Signal an active owner instead of doing the work.** With the default expiration behavior, the duplicate run can deliver its input to an owner that is still active. `resumeHook()` can race owner completion, so production code should handle `HookNotFoundError`:
...
<Callout type="warn">
Do not use this newest-wins pattern with `experimental_expires`. Cancelling the owner removes its Hook, but the token stays reserved until it expires. The new run cannot use it before then.
</Callout>
