...
**Take the token over.** Set [`experimental_force: true`](/docs/api-reference/workflow/create-hook#take-over-a-token-another-run-holds) on `createHook()` to make the new run the owner in one step, without a cancel-and-retry loop. The previous owner's `await request` rejects with [`HookForceClaimedError`](/docs/api-reference/workflow-errors/hook-force-claimed-error) and can wrap up, and `getConflict()` on the forced Hook resolves with `null`. This also works for a finished run holding a token under `experimental_minRetention`.
