...
<Callout type="warn">
Do not combine this pattern with `experimental_retention`. Cancellation makes a retained owner terminal but does not release its token claim; the claim remains until its deadline. Cancellation also does not transfer ownership by itself, so protected work must remain behind a later `getConflict()` that returns `null`.
</Callout>
