...
Outside a workflow, `await workflow.get_hook_by_token(token)` is useful as an advisory pre-check before `start()`. The lookup and start are not atomic, so the workflow must still call `hook.get_conflict()` before duplicate-sensitive work.
