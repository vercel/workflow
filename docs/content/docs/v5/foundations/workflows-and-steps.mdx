...
A run's [event log](/docs/how-it-works/event-sourcing) is bounded, and replay cost grows with the length of that log. Keep any single run's log small. If you expect any run to grow to above tens of thousands of events, split the workflow into child workflows.

If a workflow's step count scales with its input, e.g. one step per row, per document, per file — split the work into [child workflows](/cookbook/advanced/child-workflows) while the parent's log is still small, rather than sizing a single run to just fit under the limit.
