...
### How much work fits in one run

A run's [event log](/docs/how-it-works/event-sourcing) is bounded, and replay cost grows with the length of that log. Both facts point the same way: keep any single run's log small, and split larger workloads across child workflows.

**Event ceiling.** Every run has a maximum number of events; the [Vercel World](/worlds/vercel) caps a run at **25,000 events**. A run that reaches the ceiling fails with `MAX_EVENTS_EXCEEDED` and cannot be continued. Events are not steps: a step that succeeds on the first try records three (`step_created`, `step_started`, `step_completed`), retries add more, and hooks, sleeps, and webhooks each record their own. A run that is nothing but successful steps therefore exhausts the budget at roughly 8,000 steps, not 25,000 — and a retry-heavy or hook-heavy one much sooner.

Treat 25,000 as a hard backstop, not a budget to spend. If a workflow's step count scales with its input — one step per row, per document, per file — split the work into [child workflows](/cookbook/advanced/child-workflows) while the parent's log is still small, rather than sizing a single run to just fit under the limit.

**Parallel fan-out.** `Promise.all` over a large array creates every step in one run, so a wide fan-out is the fastest way to grow that log. Past roughly 100 steps in a single run, the growing log makes each step boundary progressively more expensive, and additional parallelism starts costing more than it saves. When a fan-out approaches that size, either process the items in [batches](/cookbook/common-patterns/batching) so only a chunk is in flight at a time, or give each unit of work its own child workflow so every log stays bounded.
