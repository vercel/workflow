...
- Awaiting on <LanguageContent value="ts" as="span">a promise returned by [`createWebhook()`](/docs/api-reference/workflow/create-webhook)</LanguageContent><LanguageContent value="py" as="span">the hook event returned by [`BaseHook.wait()`](/docs/api-reference/python/base-hook#wait)</LanguageContent>, which resumes the workflow when an external system passes data into the workflow.
