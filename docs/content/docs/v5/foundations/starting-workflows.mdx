...
After you define a workflow function, use the `start()` function <LanguageContent value="ts" inline>from `workflow/api`</LanguageContent> to trigger it. The function enqueues a new workflow run and returns a `Run` object for tracking its progress.
...
The [`start()`](/docs/api-reference/python/start) function programmatically triggers workflow executions from API routes or other server-side code.
...
- <LanguageContent value="ts" inline>The second argument is an array of arguments to pass to the workflow (optional if the workflow takes no arguments)</LanguageContent><LanguageContent value="py" inline>Remaining positional and keyword arguments are passed to the workflow</LanguageContent>
...
**Learn more**: <LanguageLink ts="/docs/api-reference/workflow-api/start" py="/docs/api-reference/python/start">`start()` API reference</LanguageLink>
...
When you call `start()`, it returns a <LanguageLink ts="/docs/api-reference/workflow-api/start#returns" py="/docs/api-reference/python/run">`Run`</LanguageLink> object that provides access to the workflow's status and results.
...
**Learn more**: <LanguageLink ts="/docs/api-reference/workflow-api/start#returns" py="/docs/api-reference/python/run">`Run` API reference</LanguageLink>
...
<Callout type="warn">
Waiting for <LanguageText code ts="returnValue" py="return_value()" /> can cause your API route to time out if the workflow takes a long time.
</Callout>
...
Your workflow can write to the stream using <LanguageContent value="ts" inline>[`getWritable()`](/docs/api-reference/workflow/get-writable)</LanguageContent><LanguageContent value="py" inline>[`get_writable()`](/docs/api-reference/python/get-writable)</LanguageContent>:
...
async def generate_ai_content(prompt: str):
    ...
    await stream_content_to_client(writable, prompt)
    return {"status": "complete"}

@wf.step
async def stream_content_to_client(writable: workflow.WorkflowWritable, prompt: str):
    # Stream updates as they become available
...
- Check the <LanguageLink ts="/docs/api-reference/workflow-api/start" py="/docs/api-reference/python/start">`start()` API reference</LanguageLink> for complete details
