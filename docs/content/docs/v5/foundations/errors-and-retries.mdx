...
By default, steps retry up to 3 times on arbitrary errors. You can customize the number of retries by adding a <LanguageContent value="ts" inline>`maxRetries` property to the step function</LanguageContent><LanguageContent value="py" inline>`max_retries` parameter to the step decorator</LanguageContent>.
...
When your step needs to intentionally throw an error and skip retrying, throw a <LanguageLink ts="/docs/api-reference/workflow/fatal-error" py="/docs/api-reference/python/errors#fatalerror">`FatalError`</LanguageLink>.
...
When you need to customize the delay on a retry, use <LanguageLink ts="/docs/api-reference/workflow/retryable-error" py="/docs/api-reference/python/errors#retryableerror">`RetryableError`</LanguageLink> and set the <LanguageText code ts="retryAfter" py="retry_after" /> property.
...
This final example combines everything we've learned, along with <LanguageLink ts="/docs/api-reference/workflow/get-step-metadata" py="/docs/api-reference/python/get-step-metadata"><LanguageText code ts="getStepMetadata" py="get_step_metadata()" /></LanguageLink>.
...
<Callout type="info">
  Setting <LanguageText code ts="maxRetries = 0" py="max_retries=0" /> means the step will run once but will not
  be retried on failure. The default is <LanguageText code ts="maxRetries = 3" py="max_retries=3" />, meaning the
  step can run up to 4 times total (1 initial attempt + 3 retries).
</Callout>

## Serialization failures

A step whose arguments or return value cannot be [serialized](/docs/foundations/serialization) fails like a step whose body threw a `FatalError`: the failure is deterministic, so it skips the retry loop. You can catch the error around the step call:
...
When a workflow run fails, the error includes an <LanguageText code ts="errorCode" py="error_code"/> that classifies the failure, alongside the original thrown value (preserved as <LanguageText code ts="cause" py="__cause__"/>):
...
try:
    ...
    print(error.error_code)  # e.g. "USER_ERROR", "MAX_EVENTS_EXCEEDED", or undefined
    # `error.error` is the original raised value, hydrated through the workflow
    # serialization pipeline. `__cause__` will be set to the same value if it is an Exception.
    if error.__cause__ is not None:
        print(str(error.__cause__))  # The error message
```
