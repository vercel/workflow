...
| `STREAM_ERROR` | Stream infrastructure failed while reading or writing data. This is an SDK or backend failure, not an error in your workflow code; retry the run and report persistent failures with the `runId` |
| `DEPLOYMENT_MISMATCH` | The run was delivered to a deployment other than the one it is pinned to. See [deployment-mismatch](/docs/errors/deployment-mismatch) |
