...
The run is tied to the deployment that handled this request. If you deploy a new version while the workflow is <LanguageContent value="ts" inline>[`sleep()`ing](/docs/api-reference/workflow/sleep)</LanguageContent><LanguageContent value="py" inline>[`sleep()`ing](/docs/api-reference/python/sleep)</LanguageContent>, [waiting on a hook](/docs/foundations/hooks), [retrying a step](/docs/foundations/errors-and-retries), or processing later queue messages, that existing run still resumes on the original deployment.
