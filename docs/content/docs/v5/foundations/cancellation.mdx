...
Workflow DevKit supports two cancellation mechanisms: **step cancellation** and **run cancellation**.

## Step Cancellation

Step cancellation builds on Python's `asyncio` cancellation. Steps must first opt in with `@wf.step(cancellable=True)`. Once enabled, cancelling the `asyncio` task in the workflow that is awaiting the step causes the runtime to durably forward the request to the step. Inside the step, `asyncio.CancelledError` is raised at an await point, giving the step an opportunity to clean up before exiting:
...
Cancellation is **cooperative**: `task.cancel()` requests cancellation, and `await task` waits for the step to finish. A step can catch `asyncio.CancelledError` to perform cleanup, as shown above, then re-raise it to propagate cancellation. If the step suppresses the exception, it can continue running and return normally.
...
Building on `asyncio` cancellation makes step cancellation work naturally with `asyncio.timeout()`. Simply wrap the step call in a timeout block:
...
async def fetch_with_timeout(url: str):
    try:
        async with asyncio.timeout(10):  # [!code highlight]
...
Use `asyncio.TaskGroup` to manage the step tasks, wait for the first result, and cancel the remaining tasks. The task group waits for them to finish before exiting:
...
async def first_responder(urls: list[str]):
    async with asyncio.TaskGroup() as group:
        ...
        done, pending = await asyncio.wait(  # [!code highlight]
            tasks,  # [!code highlight]
            return_when=asyncio.FIRST_COMPLETED,  # [!code highlight]
        )  # [!code highlight]

        winner = next(task for task in tasks if task in done)
        for task in pending:
            task.cancel("another request finished first")  # Cancel remaining fetches # [!code highlight]
...
async def user_cancellable_workflow(job_id: str):
    async with (
        CancelRequest.wait(token=f"cancel:{job_id}") as hook,
        asyncio.TaskGroup() as group,
    ):
        async def wait_for_cancel():
            return await hook

        work_task = group.create_task(do_expensive_work())
        cancel_task = group.create_task(wait_for_cancel())
        done, _ = await asyncio.wait(  # [!code highlight]
            [work_task, cancel_task],
            return_when=asyncio.FIRST_COMPLETED,
        )

        if cancel_task in done:  # [!code highlight]
            request = cancel_task.result()
            work_task.cancel(request.reason)  # [!code highlight]
            return {"status": "cancelled", "reason": request.reason}

        cancel_task.cancel()
        return {"status": "completed", "data": work_task.result()}
...
async def cancel(job_id: str, payload: CancelRequest):
...
When a step is cancelled by the workflow, the runtime treats the cancellation as intentional and **skips retries**. Retrying would just result in another cancellation. You don't need to manually wrap `asyncio.CancelledError` in `FatalError`.
...
@wf.workflow
async def cancellable_workflow():
    task = asyncio.create_task(cancellable_step())
    ...
    try:
        ...
        # CancelledError arrives with no retries attempted # [!code highlight]
        return {"status": "cancelled"}

@wf.step(cancellable=True)
async def cancellable_step():
    # Let CancelledError propagate when the workflow requests cancellation
    async with httpx.AsyncClient() as client:
...
Run cancellation stops an entire workflow at the next suspension point. Unlike <LanguageText code ts="AbortSignal" py="CancelledError" />, it is not cooperative. The workflow does not continue executing after cancellation.
...
```python title="app/api/cancel_run.py" lineNumbers
from vercel import workflow
...
<Callout type="info">
Run cancellation does **not** automatically <LanguageContent value="ts" inline>abort any outstanding `AbortSignal`s</LanguageContent><LanguageContent value="py" inline>issue any `CancelledError`s</LanguageContent>. Steps that are currently executing will run to completion. If you need in-flight cancellation of specific operations, use <LanguageText code ts="AbortSignal" py="task.cancel()" />.
</Callout>
...
## Step vs. run cancellation

| | Step cancellation | Run cancellation |
|---|---|---|
| **Scope** | One running step | Entire workflow run |
| **Triggered by** | `task.cancel()` in workflow code | `run.cancel()` from external code |
| **Running step** | Receives `asyncio.CancelledError` | Continues to completion |

Use step cancellation when you need to interrupt a specific step in flight. Use run cancellation when you want to stop the entire workflow.
