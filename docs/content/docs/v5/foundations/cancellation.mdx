...
No special imports and no wrapper functions: the standard `AbortController` API.
...
This is the correct behavior because an abort is an intentional cancellation, so retrying the step would result in another abort. You don't need to manually wrap abort errors in `FatalError`.
...
This is safe even if both steps have already completed, since aborting a finished operation is a no-op.
