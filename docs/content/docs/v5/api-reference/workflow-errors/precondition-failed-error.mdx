...
The same `details` may report the comparison the rejection was made on, as `recordedAtOrBelow` and `stateUpdatedAt`. The runtime uses them to check that a reloaded event log actually reached that count: a reload that comes back unchanged, or still short of it, cannot produce a different write, so the run escalates to a fresh invocation immediately rather than spending its remaining in-process restarts. A world that reports nothing here loses only that shortcut.
...
interface PreconditionFailedError {
  ...
  retryAfter?: number;
  /** Optional rejection payload. A world may put the events the client was missing here, as \`{ events, cursor }\`, so the runtime can restart its replay without re-reading the event log, and the comparison it rejected on, as \`{ recordedAtOrBelow, stateUpdatedAt }\`. */
  details?: unknown;
  /** The error message. */
  message: string;
}
