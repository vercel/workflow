...
The optimistic-concurrency guard is enabled by default; set `WORKFLOW_PRECONDITION_GUARD=0` to opt out (see [Runtime Tuning](/docs/configuration/runtime-tuning)). Event creations that carry no snapshot are never rejected with this error.
