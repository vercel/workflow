...
Cancel a workflow run. You can pass an optional free-text reason (up to 512 characters) that is recorded on the run's cancellation event and shown in the run detail view:
...
export async function POST(req: Request) {
  ...
  await run.cancel("Superseded by a newer submission"); // [!code highlight]

  return Response.json({ cancelled: true });
}
```

The reason is optional — `await run.cancel()` cancels the run without recording one.
