...
* `retention` records a data-retention preference on the run for the World to apply once the run ends. It is carried as a reserved run attribute, so it also requires a World implementing spec version 4 or later.
...
```typescript
import { start } from "workflow/api";
import { processPaymentWorkflow } from "./workflows/process-payment";

const run = await start(processPaymentWorkflow, [payload], { // [!code highlight]
  retention: "none" // [!code highlight]
}); // [!code highlight]
```

<Callout type="info">
`retention` states a preference; the World decides what it can honor. A World that does not recognize the value applies its own default, so treat it as a best-effort request rather than a guarantee. Because the preference is carried as the reserved `$retention` run attribute, a World that does not support run attributes (spec version 3 or earlier) cannot carry it at all and `start()` throws rather than silently retaining data you asked to have dropped.
</Callout>
