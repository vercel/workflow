...
* When `deploymentId` names a deployment other than the caller's, the run is stamped with the spec version the *target* deployment reports on its capability probe (capped at the caller's own), since the target is what executes it. If the target cannot be probed, the run falls back to the lowest spec version the runtime supports rather than the caller's. In that case the `attributes` and `experimental_retention` checks below apply to the target's version.
