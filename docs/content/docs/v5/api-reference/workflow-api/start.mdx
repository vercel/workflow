...
* `retention` sets a data-retention preference for after the run completes. `0` asks the World to delete user data (event payloads and stream chunks) as soon as the run completes or fails, instead of keeping it for the World's default window. On the [Vercel World](/worlds/vercel), user data is deleted but run, step and event metadata stays for your plan's default retention period. `'default'` is identical to omitting the option. The value is a duration and zero is the only one implemented, so `0` and `'default'` are the only values accepted today. Recorded as the reserved `$retention` attribute, so it needs a World implementing spec version 4 or later.
