...
`experimental_dynamic.steps` maps the aliases the source may call to steps that are already registered in your deployment. Nothing outside that map is reachable, and there is no way to define a new step from source.
...
| `experimental_dynamic.exportName` | `string` | Name of the async workflow function in the source. Defaults to `"workflow"`. |
