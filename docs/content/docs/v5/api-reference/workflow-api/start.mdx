...
`start()` also accepts a string of workflow **source** instead of an imported function, for orchestration whose shape is only known after you deploy — a workflow builder UI, a customer-defined automation, an AI-generated plan. The source is compiled and stored with the run through the same serialization path as other run payloads, including encryption when the World supplies run key material.
