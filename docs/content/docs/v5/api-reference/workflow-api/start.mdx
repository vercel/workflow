...
* In v5, you can also call `start()` directly from a workflow function to spawn a child run or continue work in a new run. See [Workflow Composition](/patterns/workflow-composition) and [Versioning](/docs/foundations/versioning).
...
<Callout type="info">
The returned `Run` object is fully functional inside a workflow. Each property access or method call (`.status`, `.returnValue`, `.cancel()`) executes as a separate step. See [Workflow Composition](/patterns/workflow-composition) for choosing between spawning a child run and awaiting a workflow function directly.
</Callout>

<Callout type="warn">
`returnValue` polls the child run every second and holds the polling step's worker slot open for as long as the child takes to finish. For long-running children, spawn without awaiting `returnValue` and have the child resume a [hook](/docs/foundations/hooks) when it completes. See the [`startAndWait()` pattern](/patterns/child-workflows).
</Callout>
