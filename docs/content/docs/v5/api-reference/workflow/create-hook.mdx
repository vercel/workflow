...
<Callout type="info">
A token can only be taken from a run whose runtime understands being taken from. When a run starts, the deployment that will execute it attests this on the run (for a cross-deployment `start()`, the target deployment answers for itself). A run without that attestation, which includes every run started by an older SDK release and every Python SDK run, would never learn that its Hook was disposed. The World declines to take its token and the forced Hook rejects with the ordinary [`HookConflictError`](/docs/api-reference/workflow-errors/hook-conflict-error) instead, exactly as if `experimental_force` had not been set. Finished runs holding a retained token are always taken over.
</Callout>
