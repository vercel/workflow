...
- **Mutating core intrinsics**: `Object`, `Array`, and `Function` (constructors and prototypes), and the global bindings for serialization-relevant built-ins (`Map`, `Set`, `Date`, `RegExp`, typed arrays, `Headers`, `URL`, and similar) are frozen. Serialization dispatches on these, so redefining them would make replay unfaithful.
