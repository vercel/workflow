...
- **Mutating core intrinsics**: `Object`, `Array`, and `Function` (constructors and prototypes), the prototypes of `Map`, `Set`, `Date`, typed arrays, and `ArrayBuffer`, and the global bindings for serialization-relevant built-ins (`RegExp`, `Headers`, `URL`, and similar) are frozen. Serialization reads through these, so redefining them would make replay unfaithful.
