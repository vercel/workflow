...
| `params.pagination.limit` | `number` | Maximum events to return when `returnAll` is false |
| `params.pagination.sortOrder` | `"asc" \| "desc"` | Event order |
| `params.resolveData` | `"all" \| "none"` | Include or omit event payload data |
| `params.returnAll` | `boolean` | Ask the World for every remaining event in one response. Supporting Worlds may cap the response and return a continuation; when honored, this overrides `pagination.limit`. |
