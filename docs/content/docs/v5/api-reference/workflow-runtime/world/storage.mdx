...
<Callout type="info">
  Step I/O is serialized using the [devalue](https://github.com/Rich-Harris/devalue) format. Use `hydrateResourceIO()` from `workflow/observability` to deserialize it for display. See [Observability Utilities](/docs/api-reference/workflow-observability).
</Callout>
...
- [Observability Utilities](/docs/api-reference/workflow-observability) — Hydrate step I/O, parse display names, decrypt data
