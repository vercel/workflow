...
- **`world.events`**: The append-only event log. This is the source of truth for all workflow state. See [Event Sourcing](/docs/advanced/event-sourcing) for background.
...
- [Event sourcing](/docs/advanced/event-sourcing): How the event log powers workflow replay and state
