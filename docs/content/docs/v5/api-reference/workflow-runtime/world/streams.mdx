...
`keyedStreamAppendVersion === 1` enables the internal exact keyed-append receipt used by recoverable public-event publishers. It is not a fallback for `write()` or `writeMulti()`: when absent, the publisher must fail before writing.

| World | Keyed append v1 |
|-------|-----------------|
| `world-local` | Unavailable; fails closed pending the cross-process canonical-EOF matrix |
