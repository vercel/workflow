...
### Exact keyed append availability

`keyedStreamAppendVersion === 1` enables the internal exact keyed-append receipt used by recoverable public-event publishers. An execution admitted to exact recovery fails before writing when it is absent. A separately authorized normal compatibility execution may use its ordinary writer only after a typed pre-commit unavailable result and only after that writer fulfills; arbitrary errors and possibly committed keyed appends never fall back.

| World | Keyed append v1 |
|-------|-----------------|
| `world-local` | Available; canonical terminal and run ownership are durable |
| `world-postgres` | Unavailable; fails closed pending service-backed ordering proof |
| `world-sim` | Available for deterministic tests |
| `world-vercel` | Unavailable; fails closed |

Ordinary unkeyed stream writes retain their existing batching behavior.
