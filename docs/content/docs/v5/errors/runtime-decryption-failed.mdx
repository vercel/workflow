...
This is an **internal SDK failure** — your workflow code never invokes the encryption primitives directly. When this surfaces, it means the ciphertext, nonce, or auth tag the SDK tried to verify is not the bytes that were originally produced.

A failure to *decrypt* persisted data is often transient: if the ciphertext arrived from a truncated or corrupted read, a fresh delivery re-fetches the event log and ref payloads from scratch and can succeed. On managed platforms (`world-vercel`), the SDK therefore re-attempts the run via queue redelivery a bounded number of times before giving up. Once that retry budget is exhausted — or immediately for *encrypt*-side failures, and for in-process worlds where there is nothing to re-fetch — the run is failed with the `RUNTIME_ERROR` classification.
...
1. **Ciphertext mutation or truncation in transit.** The encrypted payload reached the SDK with bytes that differ from what storage holds. Possible sources include a truncated HTTP response from a persisted-data ref endpoint, an edge-cache miss returning a partial 200, or a proxy drop during streaming. A truncated body whose first 4 bytes happen to still spell `encr` produces the exact "auth tag mismatch" symptom. This is the recoverable case that queue redelivery retries before failing the run.
...
On managed platforms the SDK already retries a decrypt failure via queue redelivery before giving up, so a run that reaches `failed` has exhausted those automatic attempts. You can still re-run it manually using the **Re-run** button in the Workflow Dashboard.
...
Like other `WorkflowRuntimeError` subclasses, a runtime decryption failure is **not catchable** inside your workflow function. The runtime cannot safely continue executing user code when an event-log payload can't be verified. After any bounded queue-redelivery attempts are exhausted, the entire run is marked as `failed`.
