...
This pattern is useful when you want to detect duplicate processing inside the workflow. Runtime APIs such as `resumeHook()` and `getRun()` must be called outside workflow functions, for example from an API route.

### Delegate to the Active Run

In idempotency flows, a conflict means another active run already owns the hook token. You can resume that hook to deliver the payload to the existing run, then use `getRun(error.conflictingRunId)` to wait for, stream, or cancel the active run:

```typescript lineNumbers
import { HookConflictError, WorkflowRunFailedError } from "@workflow/errors";
import { getRun, resumeHook, start } from "workflow/api";
import { processPayment } from "@/workflows/process-payment";

export async function POST(request: Request) {
  const { orderId, payment } = await request.json();
  const run = await start(processPayment, [orderId, payment]);

  try {
    return Response.json(await run.returnValue);
  } catch (error) {
    if (
      WorkflowRunFailedError.is(error) &&
      HookConflictError.is(error.cause) &&
      error.cause.conflictingRunId
    ) {
      await resumeHook(error.cause.token, payment); // [!code highlight]
      const activeRun = getRun(error.cause.conflictingRunId); // [!code highlight]
