...
Anything the replaced run still needs to publish should happen in this branch. Do not create another hook with the same token here unless this run really should take the token back: two runs forcing the same token converge on whichever registered last, and the other one receives this error again.
