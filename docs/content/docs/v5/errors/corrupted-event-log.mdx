...
### 3. Disable optional performance optimizations

If the error recurs, you can trade performance for stricter execution ordering. By default the runtime uses optimistic optimizations that allow concurrent work on a single run; for complex workflows (many parallel steps, hooks, and sleeps racing each other) these are involved in the rare event-log races behind this error. Two environment variables disable them:
...
- [`WORKFLOW_PRECONDITION_GUARD=1`](/docs/configuration/runtime-tuning#workflow_precondition_guard) makes the backend reject an event written from a stale view of the run, so the runtime reloads and retries instead of recording conflicting events.
- [`WORKFLOW_SEQUENTIAL_REPLAYS=1`](/docs/deploying/world/vercel-world#workflow_sequential_replays) (Vercel deployments only) serializes orchestrator invocations per run, so two replays can never race each other.

Set both as project-level environment variables and redeploy (`WORKFLOW_SEQUENTIAL_REPLAYS` is read at build time and runtime). Both have real costs — added run latency, delayed aborts and timeouts, and 2x queue delivery billing under sequential replays — so review their reference docs before enabling them broadly. If the error stops recurring with these set, mention that in your report below; it significantly narrows down the underlying bug.
