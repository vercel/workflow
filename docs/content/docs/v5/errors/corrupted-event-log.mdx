...
- [`WORKFLOW_SEQUENTIAL_REPLAYS=1`](/docs/deploying/world/vercel-world) (Vercel deployments only) serializes orchestrator invocations per run, so two replays can never race each other.
