...
| **Where it runs** | Co-located with your app (Vercel managed or self-host) | trigger.dev's run engine (isolated containers; microVMs on roadmap) |
...
| **Limits** | 50 MB payload; 2 GB/run | 3 MB payload / 10 MB output; 14-day max run TTL; CPU-time-based max duration |
| **Maturity** | v4 GA / v5 beta | v4 GA; YC W23; $16M Series A (Dec 2025) |
...
- **`machine` presets / custom images.** Function resources are per-deployment in the Workflow SDK (configured via your hosting platform).

---
*Compiled from public documentation as of July 2026. Verify current trigger.dev limits and pricing against [trigger.dev/docs](https://trigger.dev/docs). Not based on head-to-head benchmarks.*
