...
| --- | --- |
| Search attributes / visibility queries | Attach metadata with [`setAttributes()`](/docs/api-reference/workflow/set-attributes) (or the `attributes` option of `start()`) and filter runs by `key=value` — see [Attributes](/docs/observability/attributes) |
