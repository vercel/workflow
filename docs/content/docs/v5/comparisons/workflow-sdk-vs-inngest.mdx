...
| **Where execution runs** | Orchestration + execution co-located on your platform | **Your code runs on your infra**; Inngest invokes it per step over HTTP (Serve) or a persistent worker connection (Connect) |
| **Languages** | TypeScript / JS (Python beta) | TypeScript (GA); Python & Go (production, pre-1.0) |
...
| **AI & streaming** | `WorkflowAgent` in the AI SDK; native resumable streaming | Native AI SDK via `step.ai.wrap`; **AgentKit** multi-agent framework; durable resumable **Realtime** (`step.realtime.publish`) + `useAgent` hook |
| **Security** | VM-sandboxed code; per-run E2E encryption | Runs on your infra; signed requests; first-party E2E encryption middleware (TS); SOC 2 Type II, HIPAA add-on |
| **Portability** | Apache-2.0; World abstraction; **self-host supported** | Engine open source (SSPL → Apache 2.0 after 3 yrs); self-host via single Go binary + Postgres, but **self-hosting is unsupported** (SaaS is the default) |
| **Pricing** | SDK free; pay your platform | Per-execution: billed for the run **plus each step plus retries**; Free 50K execs, Pro from $75/mo for 1M |
| **Limits** | 50 MB payload; 2 GB/run; no step cap | 1,000 steps/function; 4 MiB step payload; 32 MiB run state; runs up to 366 days (by tier) |
| **Maturity** | v4 GA / v5 beta | TS GA; ~2022 origin; ~$34M raised |
...
- **Flow control** (concurrency, throttling, debounce, rate limiting, batching, priority). These function-level controls have no direct analog; enforce limits inside steps or debounce at the publisher before `start()`.
...
---
*Compiled from public documentation as of June 2026. Inngest tier names and pricing have shifted across sources; verify against [inngest.com/pricing](https://www.inngest.com/pricing). Not based on head-to-head benchmarks.*
