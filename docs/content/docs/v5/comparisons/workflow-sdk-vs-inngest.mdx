...
| Retry config (`retries`) / `RetryAfterError` | `maxRetries`, `RetryableError`, `FatalError` | Retries live at the step boundary; custom backoff via `retryAfter` — see [Errors & Retrying](/docs/foundations/errors-and-retries#customize-retry-behavior). |
