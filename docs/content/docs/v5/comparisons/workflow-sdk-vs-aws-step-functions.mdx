...
| Retry / Catch (`MaxAttempts`, `BackoffRate`, `IntervalSeconds`) | `maxRetries`, `RetryableError`, `FatalError`; `try/catch` for compensation | Retry logic moves to step boundaries; backoff curves via `retryAfter` — see [Errors & Retrying](/docs/foundations/errors-and-retries#advanced-example). |
