...
| Local | Drops the payloads from the run's on-disk records and empties its streams. |
| Postgres | Clears the payload columns on the run's rows and empties its stream chunks, in one transaction. |
