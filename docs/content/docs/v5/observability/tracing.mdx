...
| `workflow.lifecycle.onRunCompleted` / `workflow.lifecycle.onRunFailed` | internal | registered lifecycle handlers observe a persisted terminal transition |
...
[Lifecycle hook](/docs/observability/lifecycle-hooks) spans carry `workflow.run.id` and `workflow.name`. Their duration includes handler execution and draining background stream operations from the hydrated failure cause. Handler, hydration, and stream-operation failures add `workflow.lifecycle.error` events with `phase`, `errorName`, `errorMessage`, and `errorStack` fields, even when debug logging is disabled. These failures are isolated: a span that finishes dispatch successfully can have OK status and still contain reporting-error events. It does not change the persisted run's outcome.
