...
prerequisites:
  - /docs/foundations
related:
  - /docs/advanced/event-sourcing
  - /docs/advanced/encryption
---

Workflow SDK provides a Workflow CLI and web UI to inspect, monitor, and debug workflows. You can inspect workflow runs, steps, webhooks, [events](/docs/advanced/event-sourcing), and stream output.
...
When deployed to Vercel, workflow data is [encrypted end-to-end](/docs/advanced/encryption). Encrypted fields display as locked placeholders until you choose to decrypt them using the **Decrypt** button in the web UI or the `--decrypt` flag in the CLI.
