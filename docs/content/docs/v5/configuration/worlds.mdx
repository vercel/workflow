...
- A throttled (429) socket write or close is retried after the server's `Retry-After`, as over HTTP, for up to 30 seconds of cumulative wait, falling back to HTTP if the server closes the socket.
