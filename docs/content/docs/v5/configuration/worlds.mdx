...
- Default: `vercel` on Vercel, otherwise `local`
- Selects the World module.
...
### `WORKFLOW_TEST_LIMIT_OVERRIDES`

- Factory option: none
- CLI flag: none
- Default: unset
- Testing-only JSON object forwarded to workflow-server to tighten server-side limits on dedicated test deployments.
