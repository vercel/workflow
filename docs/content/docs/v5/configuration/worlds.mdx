...
- CLI flag: none
- Default: `http`
- Experimental stream-write transport capability. Set to exactly `ws` to attempt [`workflow-stream-ws/v1`](#workflow_streams_transport). The server authoritatively accepts or declines each upgrade; a decline uses HTTP directly for that writer lifetime. Stream reads remain HTTP and demand-driven.
...
- CLI flag: none
- Default: `1`
- Experimental WebSocket writer request depth. Supported exact values are `1`, `2`, and `4`; invalid values fail closed to `1`. This changes only opted-in WebSocket writers and does not replay requests whose outcome is unknown.
