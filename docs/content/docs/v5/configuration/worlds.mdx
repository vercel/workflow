...
- CLI flag: none
- Default: `http`
- **Experimental.** Set to `ws` to write run events over one persistent WebSocket per run instead of an HTTP request each. Event reads stay on HTTP, and the setting is ignored when the World uses `projectConfig` (the `api.vercel.com` proxy does not forward the upgrade).
- WebSocket writes are not instrumented like the HTTP path: no client span per write, and no entry in a deployment's outgoing-requests view. See [Vercel World](/worlds/vercel#workflow_events_transport).
