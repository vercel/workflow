...
<Callout type="info">
  `WORKFLOW_LOCAL_HEADERS_TIMEOUT_MS` and `WORKFLOW_LOCAL_BODY_TIMEOUT_MS` are
  inert when [`WORKFLOW_NATIVE_FETCH`](/docs/configuration/runtime-tuning#workflow_native_fetch)
  is enabled, because the Local World no longer owns the connection pool those
  bounds are set on.
</Callout>
