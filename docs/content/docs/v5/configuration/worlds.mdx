...
- Default: enabled
- Orchestrator (flow) invocations are serialized per run: each run's replays get their own queue topic and the flow trigger is generated with `maxConcurrency: 1`. Inline step executions get per-step topics and keep full parallelism. Set `0` to disable.
- A run with an armed `sleep()` or an open hook dispatches its steps to their own topics instead of running them inside the orchestrator invocation, so the run's wake-up is not stuck behind a step body.
