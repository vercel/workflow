...
- Only applies when `start()` is called without an explicit `onRestart` mode (e.g. a test harness calling it directly). The framework startup wiring goes through `ensureWorldStarted()`, which always passes a mode that overrides this option — use [`WORKFLOW_SKIP_BOOT_RECOVERY=1`](/docs/configuration/runtime-tuning#workflow_skip_boot_recovery) to skip recovery there.
