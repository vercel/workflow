...
- CLI flag: none
- Default: `http`
- Experimental stream-write transport capability. Set to exactly `ws` to advertise support for [`workflow-stream-ws/v1`](#workflow_streams_transport) when it becomes available. The server authoritatively accepts or declines an upgrade; a decline uses HTTP directly. Stream reads remain HTTP and demand-driven.
