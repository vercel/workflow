...
- Default: `3`
- Consecutive stream-reader sessions that may end without delivering a chunk while the stream reports a chunk at the resume index. Past this, the read fails with a `StreamError` naming the unreadable index, so the consumer can resume after it with `startIndex` instead of waiting indefinitely.
