...
- Default: enabled
- Gives each kind of entity a workflow creates — steps, waits, hooks, attribute writes, abort controllers, stream IDs — its own sequence of correlation IDs.
...
- IDs remain ordered within a kind, so hooks are still listed in creation order.
- Correlation IDs of runs created before the setting changed are not affected: a run keeps replaying on the deployment it started on. On platforms without that guarantee, only change it while no runs are in flight — a replay that switches schemes mid-run cannot consume its own earlier events.
- Set `0` to disable.
