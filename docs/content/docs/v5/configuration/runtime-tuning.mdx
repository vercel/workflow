...
- It composes with the [batched fan-out fold](/docs/changelog/batched-event-writes). Inside the fold, each step's message is published as soon as its input is serialized, concurrently with the `createBatch` commits, rather than after the chunk carrying its create commits. Outside the fold (a suspension with hook or attribute writes, a World without `createBatch`, a run that predates slot identity), the write and the publish are issued in parallel. On 32-branch fan-outs, queue messages used to leave 100-400 ms after the suspension began because they waited on chunk commits of 155-182 ms each; they now leave as soon as the inputs are serialized.
- Why it is enabled now: it shipped off by default while a World could refuse a stale `step_created` with a `412` ([stale write](#stale-reads-and-why-nothing-has-to-be-rejected)) after the payload-carrying message was already out, with nothing ordering that verdict before the consumer's recovery. No shipped World refuses a stale write any more (see the next section), the Vercel backend skips its precondition check entirely for slot-identity runs, and the only refusal left, a duplicate-create `409`, means a concurrent writer already created the step, whose own dispatch the out message is deduplicated against by the shared idempotency key. The publish is only initiated before the create, not completed before it: the two run concurrently and both must succeed before the invocation acknowledges its message, so a durable `step_created` does not by itself prove its message was published, and a pending step is still re-dispatched on the next replay (deduplicated by the idempotency key).
...
- Set `0` (or `false`) to disable it and restore the sequential create-then-publish dispatch, the only ordering that gives the message a happens-after edge over the create's verdict, for a World that does refuse stale writes.
