...
- The default leaves headroom under the Vercel World's 60s per-request HTTP timeout, so the budget expires as a response carrying the run's current status rather than as a client-side timeout. Raising it past that headroom has no effect: the wait is clamped to stay inside the request timeout.
