...
- Default: enabled
- Only takes effect on a World whose queue redelivers unacknowledged messages on its own (it declares `capabilities.queueRedeliversUnacked`; `@workflow/world-vercel` does). A pending step whose latest `step_started` is bare (written by a queue delivery of the step message, not by an inline owner) and has no terminal event is being executed by a delivery that has not acknowledged its message yet, so a replay arms a delayed backstop wake for the remaining ownership lease instead of re-sending the step message. Without this, every replay of a run with such steps re-sends one message per step; the queue deduplicates them, but each is still a send. Steps that were created but never started are re-enqueued immediately either way, because `step_created` does not prove the step's message was ever sent.
