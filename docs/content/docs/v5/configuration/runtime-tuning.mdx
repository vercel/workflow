...
- An open wait (a `sleep()` whose timer has not fired, including one that lost a `Promise.race()` against a hook) keeps the per-step event-log delta optimization and turbo's forced optimistic start off only while its target time is within this many milliseconds of the runtime's clock. A wait due further out cannot complete during the step boundary being scheduled, so it does not cost the extra `events.list()` per step or exit turbo. A `wait_created` whose target time cannot be read is treated as due now.
- Sized to cover the step boundary's own exposure window plus clock skew between the runtime and the wait timer's writer, with an order of magnitude to spare. A very large value restores the older behavior of gating on any open wait. Open hooks are unaffected: they gate regardless of this setting.
