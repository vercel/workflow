...
- Only engages for a single sequential inline step with no open hook or wait, and only when concurrent orchestrator invocations are ruled out — on turbo's first delivery, or under [`WORKFLOW_SEQUENTIAL_REPLAYS=1`](/worlds/vercel#workflow_sequential_replays).
