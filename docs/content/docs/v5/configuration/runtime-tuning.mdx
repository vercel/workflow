...
- Default: enabled (parallel durable hook resume on)
- Resuming a hook durably writes `hook_received` and publishes a payload-less workflow wake in parallel. The consumer waits for the matching event before replaying or acknowledging the wake. `resumeHook()` resolves only after both operations succeed.
- The runtime falls back to write-then-publish when the consumer does not advertise the wake barrier or the backend does not attest atomic `(runId, resumeId)` claims.
- A refused event write, including disposal or an ended run, surfaces as `HookNotFoundError` and the call does not report success.
