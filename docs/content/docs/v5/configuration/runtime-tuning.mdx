...
- Stops forcing optimistic inline step start once the run creates a hook or wait — unless `WORKFLOW_SEQUENTIAL_REPLAYS=1` serializes the run's orchestrator invocations (see [Vercel World](/worlds/vercel#workflow_sequential_replays)) on a World whose queue declares `maxConcurrency` support (the Vercel World does), in which case it stays active.
