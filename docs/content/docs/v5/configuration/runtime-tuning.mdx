...
- Every replay-context event creation names the position it replayed from (`eventCount`, the number of events the replay had loaded). A World that fences on it rejects a creation with 412 ([`PreconditionFailedError`](/docs/api-reference/workflow-errors/precondition-failed-error)) when the log already held more than that.
