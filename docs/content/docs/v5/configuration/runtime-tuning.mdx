...
- Set `0` to go back to that library. See the trade-offs below before leaving this on.
...
Connection pooling, keep-alive, and the request, header, and body deadlines are preserved: they are configured on Node's agents and passed per request from the same settings the Worlds already use.
