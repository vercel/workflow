...
- The Local and Postgres Worlds also use it as the maximum number of events returned by one `returnAll` request. If more events exist, the response includes `hasMore: true` and a continuation cursor.
