...
- Resuming a hook persists the `hook_received` event and publishes the workflow invocation concurrently, cutting a round trip off resume latency. The runtime falls back to the sequential path automatically when the consumer or backend does not attest dedup support, so a lost event write still resumes the run.
