...
- Default: disabled
- When a suspension hands newly created steps to the queue, the runtime publishes each step's execution message without waiting for its `step_created` event write to commit, instead of sequencing the two. The message carries the serialized step input (`stepInput`), so a delivery that arrives before the write lands, or after a transient write failure (429 / 5xx / transport), still executes the step: the consumer's bare `step_started` finds no step, and it sends the start once more as a lazy `step_started` carrying that input, which every World turns into an atomic create-and-start (one round trip, the same write the inline path uses). If that lazy start loses to the producer's create landing in between (a `409`), one more bare start runs the step. A lazy `step_started` on a step that already exists is always a `409`, so the lazy start is only ever the recovery for a step-missing bare start, never the first attempt, and retries and redeliveries of a materialized step keep the bare start. A `409` on a start is never taken as "already started" by itself, because a World whose start races the create it is conditioned on can refuse a start it should have accepted (the Vercel backend phrases that one as a step that is `pending` yet cannot be started): the consumer reads the step and lets the entity decide. A `running` or finished step acknowledges the delivery as the loser, a missing step is materialized from the message's input, and a step that is still `pending` gets its start retried once; if it is still `pending` after that, the delivery fails so the queue redelivers it, rather than acknowledging a step nobody started, which would strand it, since the queue deduplicates every later publish of that step against the acknowledged message. This mirrors resilient start (`runInput`) and the legacy lazy hook resume's `hookInput` (which current producers no longer send; see [durable hook resume](/docs/changelog/lazy-hook-resume)).
- It composes with the [batched fan-out fold](/docs/changelog/batched-event-writes). Inside the fold, the messages of every step whose input can ride them are published in one batch as soon as the inputs are serialized, concurrently with the `createBatch` commits, rather than after the chunk carrying each create commits. Outside the fold (a suspension with hook or attribute writes, a World without `createBatch`, a run that predates slot identity), the write and the publish are issued in parallel. On 32-branch fan-outs, queue messages left 100-400 ms after the suspension began because they waited on chunk commits of 155-182 ms each; enabling this sends them as soon as the inputs are serialized.
- Why it is still off by default: publishing before the create takes away the implication "a step message exists, so its `step_created` is durable", and that implication is what makes the runtime's inline step claim exclusive. A replay that loads its log and finds a step uncreated may claim it and run it inline; that is only sound while no message for the same step can already be in flight. Once messages go out first, a redelivered or concurrent replay (during a chunk commit, or after a tolerated transient `step_created` failure) can inline-claim a step whose message is already on its way to a consumer, and the consumer's `step_started` is accepted on a running step, because retries need it to be. The step body then runs twice. Closing that needs an ownership fence on the consumer's start, which no World offers today. The older reason — a World refusing a stale `step_created` with a `412` ([stale write](#stale-reads-and-why-nothing-has-to-be-rejected)) after the payload-carrying message was already out — no longer applies: no shipped World refuses a stale write, and the Vercel backend skips its precondition check entirely for slot-identity runs.
- The publish is initiated before the create, not completed before it: the two run concurrently and both must succeed before the invocation acknowledges its message, so a durable `step_created` does not by itself prove its message was published, and a pending step is still re-dispatched on the next replay (deduplicated by the idempotency key).
- Even when enabled, a step whose serialized input is too large to inline on the queue message (128 KiB), or whose run's queue transport cannot carry binary payloads (pre-CBOR spec versions), keeps the sequential create-then-publish dispatch: inside the fold its message is published once its chunk commits.
- Producer-side recoveries on the parallel (non-fold) path are reported on the suspension span as `workflow.step.resilient_dispatch_recovered`; a consumer that materialized the step from the message reports `workflow.step.resilient_dispatch_materialized`.
- Set `1` to enable it.
