...
- A step that has already started gets the same treatment on a later deadline: it is presumed alive for the inline ownership lease above, and only past the lease does its re-dispatch move to a fresh key. This is what recovers a step whose invocation disappeared mid-body. Once past that point, the interval here is also how often the re-dispatch is retried.
