...
- Adds a backend-side check to every guarded event creation. Best-effort: backends without guard support ignore the snapshot, and the guard fails open.
- Also enabled by [`WORKFLOW_SAFE_MODE=1`](/docs/deploying/world/vercel-world#workflow_safe_mode) when not set explicitly.
