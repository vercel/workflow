...
- Default: `0`
- How many times a run numbered by `WORKFLOW_SLOT_IDENTITY` may re-send an event creation that lost its position, before the rejection propagates and the run replays.
- Zero means never. A rejected position proves the replay decided from an event log missing at least one event, and re-sending the same write to a free position commits that decision anyway — the missing event may be the one that would have taken the workflow down another branch.
- Raise it for a run that would rather keep a batch of writes contiguous than replay, accepting that a write may land under a decision the replay would have revised.
