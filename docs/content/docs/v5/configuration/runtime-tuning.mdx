...
- Step inputs made of plain data (objects, arrays, primitives) and standard built-ins (`Map`, `Set`, `Date`, `RegExp`, typed arrays, `ArrayBuffer`, `URL`, `Headers`) remain retainable: serialization reads them through captured intrinsics, so patching or polyfilling built-in prototypes does not affect retention. Anything whose serialization executes workflow code or perturbs engine state the workflow can observe — getters, proxies, custom class serializers, and `Error` instances (serializing one materializes its lazy `stack`) — falls back to replay for that boundary.
