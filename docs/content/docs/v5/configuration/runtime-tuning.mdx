...
- Suspensions involving hooks, waits, or attributes, workflows that used host-timed async APIs (`Atomics.waitAsync`, async `WebAssembly` compilation), and any replay divergence always fall back to a full replay.
