...
- When enabled, the runtime also keeps the per-step event-log delta optimization (consuming the delta returned by a step's terminal write instead of issuing an extra `events.list` per step) active while the run has an open hook; without the guard, an open hook disables it.
...
- Stops forcing optimistic inline step start once the run creates a hook or wait — unless `WORKFLOW_SEQUENTIAL_REPLAYS=1` serializes the run's orchestrator invocations (see [Vercel World](/worlds/vercel#workflow_sequential_replays)), in which case it stays active.
