...
- Step inputs made of plain data (objects, arrays, primitives) and standard built-ins (`Map`, `Set`, `Date`, typed arrays, `ArrayBuffer`) remain retainable. Anything whose serialization could execute workflow code — accessors, proxies, functions, custom class serializers, `RegExp` — falls back to replay for that boundary, as does any input if the prototype members serialization relies on have been modified.
