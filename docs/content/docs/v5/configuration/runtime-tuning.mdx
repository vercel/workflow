...
- An open wait (a `sleep()` whose timer has not fired, including one that lost a `Promise.race()` against a hook) keeps the per-step event-log delta optimization and turbo's forced optimistic start off only if it can fire while the current invocation is still alive: its target time must fall before the end of the invocation's inline window (`WORKFLOW_V2_TIMEOUT_MS`, or the budget derived from the platform's function deadline) plus this allowance for clock skew between the runtime and the wait timer. A wait due later than that does not cost the extra `events.list()` per step or exit turbo. A `wait_created` whose target time cannot be read is treated as due now.
- Raising the function's duration widens the window automatically. A very large value here restores the older behavior of gating on any open wait. Open hooks are unaffected: they gate regardless of this setting.
