...
- Maximum time a step waits for stream chunks it wrote to be acknowledged by the Workflow server before recording `step_completed`. A timeout fails the step instead of exposing a completion while data remains client-side. This is independent of the short inline-loop heuristic for writer-lock release.
