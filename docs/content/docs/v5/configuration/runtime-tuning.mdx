...
- Has no effect when `WORKFLOW_NATIVE_FETCH` is enabled, which takes the whole HTTP/2 path away.

### `WORKFLOW_NATIVE_FETCH`

- Default: enabled
- Makes the Vercel and Local Worlds issue their HTTP requests on whatever connection pool the runtime's own `fetch()` uses, instead of the connection pools those Worlds normally build for themselves.
- Set `0` to keep the Worlds' own pools. Reach for this if the runtime's `fetch()` is slower or less reliable for your deployment, and see the trade-offs below before leaving it on.
- Use it when the Worlds' own pools are not an option: a bundler that mangles their transport dependency, or a runtime that does not ship a working copy of it.

Enabling it drops the per-call-site tuning those pools carry:

- Event-log requests lose HTTP/2, so concurrent reads and writes no longer share one connection, and the enlarged HTTP/2 receive windows no longer apply. This is the largest difference, and it slows down replays that read a big event log.
...
- Connection pooling, keep-alive, and per-request timeouts follow the runtime's defaults rather than the Worlds' settings. On the Local World this makes `WORKFLOW_LOCAL_HEADERS_TIMEOUT_MS` and `WORKFLOW_LOCAL_BODY_TIMEOUT_MS` inert, so a queue handler that accepts a request and never responds is no longer redelivered on those bounds.
