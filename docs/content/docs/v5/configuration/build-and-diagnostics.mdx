...
- The `app` and `pages` directories (`src/` variants included), so a route you create is picked up even though nothing imports it yet, along with root entrypoints such as `middleware.ts` and `instrumentation.ts`.

A directory your app never imports is not watched, so files there never produce workflow bundles or rebuilds. Import one from a page and the next rebuild brings it into the watched set. Files are only ever watched through the directory that contains them, so the watcher's cost scales with the number of watched directories rather than the number of source files.
