...
**Why this transformation?** When a workflow executes, it needs to replay past steps from the [event log](/docs/advanced/event-sourcing) rather than re-executing them. The `WORKFLOW_USE_STEP` symbol is a special runtime hook that:
...
- Results are cached in the [event log](/docs/advanced/event-sourcing) after first execution
...
**For framework authors**: Learn how to integrate these transformations into your framework in [Building Framework Integrations](/docs/advanced/framework-integrations).
