...
When `new AbortController()` is called in a workflow, an internal hook is created, similar to calling `createHook()`. This hook is registered in the workflow's invocations queue and produces events in the [event log](/docs/advanced/event-sourcing):
...
- [Event Sourcing](/docs/advanced/event-sourcing): How the event log works
