...
```python filename="app/workflows/ai_content_workflow.py" {4}
...
```python filename="app/api/generate.py" {6,9,12}
...
```python filename="app/workflows/streaming.py" {6,9}
from app.workflow import wf
from vercel import workflow
...
```python filename="app/api/stream.py" {8}
