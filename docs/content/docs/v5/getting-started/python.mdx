...
```python title="app/workflows/report.py"
import asyncio
...
- `workflow.now()` is a deterministic substitute for `datetime.datetime.now`. It returns the time that the last workflow event occured at.
...
```python title="app/workflows/approval.py"
import typing
...
```python title="app/api/resume.py"
...
Steps can stream progress while a workflow is running. Streams are assoicated with workflow runs, can be acquired from inside a workflow or inside a step, and can be passed to workflows and steps as arguments. If a type is specified when getting the stream (or on the type annotation of a step or workflow it is passed to), then Pydantic will be used to encode and decode the type.

```python title="app/workflows/streaming.py"
import pydantic
