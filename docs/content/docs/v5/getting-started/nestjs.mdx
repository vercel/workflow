...
Use this when your NestJS project compiles CommonJS via SWC. Generate the SWC
configuration with CommonJS output:
...
<Callout type="info">
`WorkflowModule.forRoot()` reads `module.type` from `.swcrc` and `outDir` from
`tsconfig.json`, so CommonJS projects do not need duplicate Workflow options
unless they use a non-standard compiler setup.
</Callout>
...
<Callout type="info">
The `.js` local import specifiers in this example are the ESM form. CommonJS
projects can use the import style expected by their NestJS compiler setup.
</Callout>
...
WorkflowModule.forRoot({
  ...
  // Defaults to nest-cli.json sourceRoot, then ['src'].
  // Prefer sourceRoot unless you need a Workflow-specific override.
  dirs: ['src'],

  // Project root used for monorepo workspace imports.
  // Defaults to the nearest workspace root, then workingDir.
  projectRoot: '/repo',
  ...
  // SWC module type override. Defaults to .swcrc module.type, then
  // tsconfig module, then 'es6'.
  moduleType: 'es6',

  // Compiled .js output directory override. Defaults to tsconfig outDir,
  // then 'dist'. Only used when moduleType resolves to 'commonjs'.
  distDir: 'dist',
