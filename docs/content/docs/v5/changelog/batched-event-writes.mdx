...
**Publish-first, per-chunk continuation.** A folded step whose input rides its queue message is published as soon as that input is serialized, concurrently with the `createBatch` commits, so the messages are out before any chunk has committed. Each chunk's remaining follow-on work starts the moment **that chunk** commits, not when the whole fold does: the messages of steps whose input could not ride the queue message (too large, or resilient dispatch disabled) publish right off their own chunk's commit, and only the chunk carrying the inline pairs gates the replay's continuation. Trailing chunks' commits and every publish are joined before the invocation can acknowledge its message, so the durability contract ("every create durable before ack") is unchanged.
