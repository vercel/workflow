...
Ownership cannot be unconditional. Suppose replays deferred to the owner forever: an owner
that dies *and exhausts its redelivery budget* (poison delivery, queue retention limits,
operator interference) leaves a stamped, non-terminal step that no message will ever
drive. Every future wake would defer to a ghost. Unbounded ownership converts "duplicate
execution" into "permanently wedged run" — a strictly worse failure mode.
