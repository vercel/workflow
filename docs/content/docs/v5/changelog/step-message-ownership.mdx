...
| Created, latest start bare, no `step_retrying`, World declares `capabilities.queueRedeliversUnacked` | **Ensure backstop wake** (same shape, keyed to the bare start's timestamp). The bare start was written by a queue delivery of the step message that has not acked yet, so the queue itself redelivers it if that consumer dies; the immediate re-send was duplicate traffic. Once the lease is spent the backstop falls through to the row below. `WORKFLOW_QUEUE_OWNED_BACKSTOP=0` disables this row. |
...
- Span attributes on `workflow.execute`:
  ...
  `workflow.queue_ownership.backstop_wakes_armed` (a replay suppressed an immediate
  requeue of a queue-owned running step on a World whose queue redelivers unacked
  messages).
