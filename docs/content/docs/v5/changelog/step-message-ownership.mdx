...
| Created, latest start bare, no `step_retrying`, run at spec version 6 or later, World declares `capabilities.queueRedeliversUnacked` | **No send for the step.** The bare start was written by a queue delivery of the step message that has not acked yet, so the queue itself redelivers it if that consumer dies; the immediate re-send was duplicate traffic. After the loop the pass arms **one** run continuation for all such steps, `delaySeconds = leaseRemaining` of the latest bare start, `idempotencyKey = runId:queue-backstop:<latest bare start timestamp>`, and skips even that send when this invocation already armed the same epoch. Once the leases are spent the wake falls through to the row below for whatever is still pending. The spec-version gate keeps a legacy unstamped inline start (which the replay contract tolerates) on the conservative row. `WORKFLOW_QUEUE_OWNED_BACKSTOP=0` disables this row. |
...
- Span attributes on `workflow.execute`:
  ...
  steps), `workflow.inline_ownership.backstop_wakes_armed` (a replay suppressed an
  immediate requeue of an inline-owned step) and
  `workflow.queue_ownership.backstop_wakes_armed` (`1` when a replay pass armed the run's
  single backstop wake for its queue-owned running steps on a World whose queue
  redelivers unacked messages; absent when it re-sent their messages or had already
  armed that wake on an earlier pass).
