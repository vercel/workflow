...
4. **Queueing is unconditional**: For every pending step except the one being executed inline, the handler enqueues a background step message with `idempotencyKey: correlationId`. This makes crash recovery work: if a prior handler wrote `step_created` but crashed before enqueueing, a later handler will enqueue the orphaned step. Concurrent handlers' redundant enqueues deduplicate on the idempotency key. One later refinement: on a World whose queue redelivers unacknowledged messages (`capabilities.queueRedeliversUnacked`), a step whose bare `step_started` is already in the log is being run by a queue delivery that has not acknowledged yet, so the handler does not re-send its message and instead arms a single delayed backstop wake per replay pass covering every such step. See the [dispatch decision table](/docs/changelog/step-message-ownership#the-dispatch-decision-table). Created-but-never-started steps are still enqueued unconditionally.
...
Step queueing remains unconditional for created-but-never-started steps (covers crash recovery: if a prior handler wrote `step_created` but crashed before queueing, a later handler will queue it; idempotency keys dedupe redundant queues across concurrent handlers). Steps a queue delivery has already started are covered by one delayed backstop wake per replay pass instead on Worlds whose queue redelivers unacknowledged messages; see [Inline step message ownership](/docs/changelog/step-message-ownership#the-dispatch-decision-table) and `WORKFLOW_QUEUE_OWNED_BACKSTOP` in [Runtime tuning](/docs/configuration/runtime-tuning#workflow_queue_owned_backstop).
