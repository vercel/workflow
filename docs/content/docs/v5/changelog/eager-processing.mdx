...
Wait continuations are likewise deduplicated, keyed on the wait's correlation ID: while a wait is pending, every replay pass over the run re-observes it and would otherwise enqueue another delayed continuation. Near-elapsed waits (≤2s remaining) are enqueued without the key — a continuation delivered marginally early re-observes its wait as still pending and must be able to enqueue a fresh short-delay retry, which a world's dedupe window would otherwise drop, stalling the run.
