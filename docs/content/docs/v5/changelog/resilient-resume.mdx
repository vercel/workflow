...
<Callout type="info">
  Superseded by [durable hook resume](/docs/changelog/lazy-hook-resume): the
  producer now always writes `hook_received` and resolves only after both that
  durable write and the workflow wake succeed. Compatible consumers receive a
  payload-less wake carrying the expected `resumeId`; older consumers use
  write-then-publish. The queue-carried payload and `resilientResume` behavior
  described below are historical.
</Callout>
