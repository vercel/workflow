...
| `hook.metadata` is a Promise on hooks returned by `getHookByToken()` and `resumeHook()` | Write `await hook.metadata` where you read it. Like `run.returnValue`, the getter is non-enumerable, so it is missing from `{ ...hook }` and `JSON.stringify(hook)`: forward the awaited value explicitly. This makes the lookup a single read, because decrypting metadata needs the run's payload keys and that key round trip is now paid only by code that reads it. The `Hook` type is exported from `workflow/api`; the World-level record from `world.hooks.getByToken()` is unchanged. See [`getHookByToken()`](/docs/api-reference/workflow-api/get-hook-by-token). |
