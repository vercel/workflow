...
Everything that crosses a workflow/step boundary is serialized, and v5 widens what survives the trip. Errors retain their message, stack, and `cause` chain through `WorkflowRunFailedError.cause`. Built-in error types are restored, and custom classes can be restored with `WORKFLOW_SERIALIZE` / `WORKFLOW_DESERIALIZE`. Plain user Error subclasses become generic errors with their `name` preserved. Constructor identity can differ across bundled module copies, so use SDK `.is()` guards and structural properties instead of relying on `instanceof`:
...
try {
  ...
  if (
    ...
    error.cause !== null &&
    "name" in error.cause &&
    error.cause.name === "PaymentDeclinedError" // [!code highlight]
  ) {
    // Classify the failure without depending on a shared constructor.
  }
}
```
