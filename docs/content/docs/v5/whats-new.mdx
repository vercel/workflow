...
Cancelling from the outside gained detail too: [`run.cancel()`](/docs/api-reference/workflow-api/get-run) accepts a `cancelReason`, recorded on the cancellation event and shown in the run detail view.
...
| Step queue topics are retired | The `'step'` queue kind is gone: queued steps travel on the workflow topic (carrying `stepId`/`stepName` in the payload) and execute in the combined flow handler. A World that provisioned separate `__wkf_step_*` topics can drop them. |
...
| `createLocalWorld()` and `createVercelWorld()` removed | Import `createWorld()` from the same package instead — for example `import { createWorld } from '@workflow/world-local'`. The arguments are unchanged. |
| `stepEntrypoint` removed from `workflow/runtime` | Steps execute through the combined workflow handler generated by the framework integrations; use `getWorldHandlers()` for custom hosts. |
