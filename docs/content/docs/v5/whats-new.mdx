...
Everything that crosses a workflow/step boundary is serialized. In v5, built-in errors such as `TypeError` preserve their type, message, stack, and `cause` chain through `WorkflowRunFailedError.cause`. Plain user-defined Error subclasses preserve their name but revive as generic errors unless they implement [custom serialization](/docs/foundations/serialization). Bundled module copies can still have distinct constructors, so use SDK error `.is()` helpers instead of `instanceof`:
...
try {
  ...
  if (
    ...
    FatalError.is(error.cause) // [!code highlight]
  ) {
    console.error(error.errorCode, error.cause.message);
  }
}
```
