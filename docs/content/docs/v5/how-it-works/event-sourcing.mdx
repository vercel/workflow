...
When a workflow ends, its Hooks are removed and cannot receive more payloads. Their tokens are reusable by default. With `experimental_retention`, a token can stay reserved until its deadline even though the Hook is gone. An explicit `hook_disposed` event always releases the token immediately.
