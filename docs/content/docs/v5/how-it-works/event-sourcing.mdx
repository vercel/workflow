...
When a workflow ends, its Hooks are removed. Their tokens normally become available again, but `experimental_minRetention` can delay that. A `hook_disposed` event makes a token available immediately.
