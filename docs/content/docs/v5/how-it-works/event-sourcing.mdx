...
When a workflow ends, its Hooks are removed and cannot receive more payloads. Their tokens expire by default. With `experimental_expires`, a token can stay reserved after its Hook is gone. An explicit `hook_disposed` event always expires the token immediately.
