...
Counting the events above is how you size a run. A step that succeeds on the first attempt contributes three events; a retry adds a `step_retrying` and another `step_started`; a sleep contributes two, and a hook at least two.

A run's log is capped — the [Vercel World](/worlds/vercel#per-run-event-limit) fails anything past its ceiling with `MAX_EVENTS_EXCEEDED`. If you expect a run to accumulate tens of thousands of events, split the work into [child workflows](/cookbook/advanced/child-workflows).
