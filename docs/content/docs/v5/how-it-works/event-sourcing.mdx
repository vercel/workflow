...
- `hook_conflict`: records a failed acquisition of the hook's token. Acquisition is idempotent — a run re-creating a hook it already owns converges on the existing `hook_created` rather than conflicting — so a `hook_conflict` cannot follow a `hook_created` for the same hook.
