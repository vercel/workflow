...
flowchart LR
    ...
    A --> C["Workflow Mode"]
    A --> D["Client Mode"]
    B --> E["__step_registrations.js<br/>(Imported by flow)"]
    C --> F["flow.js<br/>(Combined Execution)"]
...
When you build your application, the Workflow SDK generates a combined flow handler in `.well-known/workflow/v2/` and a webhook handler in `.well-known/workflow/v1/`:
...
1. **Look in `.well-known/workflow/`**: Check the generated v2 `flow.js`, v2 `__step_registrations.js`, v1 `webhook.js`, and other emitted debug files.
