...
On Vercel, configure the flow function as the sole queue consumer. `getWorkflowQueueTrigger()` handles the optional queue namespace and the sequential-replay behavior (`WORKFLOW_SEQUENTIAL_REPLAYS`, on by default).
