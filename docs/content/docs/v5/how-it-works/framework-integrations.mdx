...
Your integration must expose two HTTP endpoints. The generated handlers manage all protocol details - you just route requests.
...
Route the two endpoints to the generated handlers. The exact implementation depends on your framework's routing API.
...
- **Framework middleware** - Add authentication (API keys, JWT, OIDC) in front of the `/.well-known/workflow/v2/flow` and `/.well-known/workflow/v1/webhook/*` routes
