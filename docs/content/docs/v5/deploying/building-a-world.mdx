...
**Automatic Hook Cleanup:** When a workflow ends, remove its live Hooks. Expire each token by default. If your World supports `experimental_expires`, keep the token reserved until `tokenReusableAfter`.
