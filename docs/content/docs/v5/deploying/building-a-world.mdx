...
**Automatic Hook Cleanup:** When a run ends, remove its live Hooks. Make each token available unless its `tokenReusableAfter` is still in the future. A `hook_disposed` event always makes the token available immediately.
