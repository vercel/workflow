...
The Vercel World requires no configuration when deployed to Vercel — the platform injects everything the runtime needs (`VERCEL_DEPLOYMENT_ID`, `VERCEL_PROJECT_ID`, per-request OIDC tokens, and the deployment encryption key).

The `WORKFLOW_VERCEL_*` variables below configure **tooling that talks to the Vercel World from outside a deployment** — the `workflow` CLI, the web UI, and tests. They have no effect inside a deployed Vercel function (the runtime warns if they're set there); the CLI infers all of them from your `.vercel` directory and Vercel CLI login when unset.
...
Maximum delay of a single queue message; longer sleeps chain messages. Read inside the deployment. Default: `82800` (23 hours).

### Programmatic configuration

`createVercelWorld()` accepts explicit API configuration. It does not read `WORKFLOW_VERCEL_*` automatically, so pass the environment values yourself when you want a configured World module:
...
export default createVercelWorld({
