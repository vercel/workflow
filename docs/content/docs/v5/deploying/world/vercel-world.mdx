...
In a Vercel deployment, you do not configure the Vercel World yourself. The platform injects everything the runtime needs (`VERCEL_DEPLOYMENT_ID`, `VERCEL_PROJECT_ID`, per-request OIDC tokens, and the deployment encryption key).
