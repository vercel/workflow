...
The Vercel World runs in every [Vercel Function region](https://vercel.com/docs/regions). Each workflow run is pinned to a single region at creation time, and everything about that run stays there: its stored state, its step execution, and its streams are all served region-locally — no cross-region round trips on the hot path.

<Callout type="info">
  Multi-region requires `workflow` **5.x** (currently in beta).
  {/* TODO: pin the exact minimum version once released */}
...
- A run's region is fixed for its entire lifetime. Reads, hook resumes, and stream consumers can come from anywhere — the platform routes them to the run's region automatically.
