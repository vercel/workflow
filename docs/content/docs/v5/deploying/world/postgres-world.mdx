...
Maximum size of the internal `pg.Pool` used when `createWorld()` constructs the pool. Default: `max(10, queueConcurrency + 2)`, which is `52` when `queueConcurrency` is `50`.

Precedence: `createWorld({ maxPoolSize })` > `WORKFLOW_POSTGRES_MAX_POOL_SIZE` > `max(10, queueConcurrency + 2)`.
