...
In **development**, `ensureWorldStarted()` cancels in-flight runs from the previous session instead of recovering them (your workflow code has likely changed, so replaying old runs would diverge). Pass your framework's dev flag so it can tell dev from production — see [Development vs production](/docs/deploying/recovering-in-flight-runs#development-vs-production).
