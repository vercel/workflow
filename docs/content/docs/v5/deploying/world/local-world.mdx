...
Options passed to `createLocalWorld()` take precedence over the environment variables above. Export the configured World from a module and point `WORKFLOW_TARGET_WORLD` at that module:

```typescript title="my-world.ts" lineNumbers
import { createLocalWorld } from "@workflow/world-local";

export default createLocalWorld({
