...
- Any fan-out approaching ~100 steps, where a single flat `Promise.all` would make the run's event log grow faster than the parallelism is worth
...
- **Batching bounds concurrency, not the run's total size**: Every batch still appends to the same event log — roughly three events per successful step — so a long enough list will walk a single run toward the [25,000-event ceiling](/docs/foundations/workflows-and-steps#how-much-work-fits-in-one-run) no matter how small the batches are. When the item count scales with your input, spawn a [child workflow](/cookbook/advanced/child-workflows) per batch instead, so each run's log stays bounded.
