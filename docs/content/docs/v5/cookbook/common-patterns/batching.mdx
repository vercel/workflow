...
- **Batching bounds concurrency, not the run's total size**: Every batch still appends to the same [event log](/docs/how-it-works/event-sourcing#how-fast-a-log-grows), so a long enough list walks one run toward its [event limit](/worlds/vercel#per-run-event-limit) no matter how small the batches are. Bundle more items per step, or spawn a [child workflow](/cookbook/advanced/child-workflows) per batch, when a single run would accumulate tens of thousands of events.
