...
- **Bound the parallelism**: `Promise.all` over a large array fans out one concurrent step per item, all in the same run. When concurrency gets high, batch the array into chunks or bundle several items into each step (see [Batching](/cookbook/common-patterns/batching)) so fewer, larger units of work are in flight. Splitting a wide fan-out across [child workflows](/cookbook/advanced/child-workflows) keeps each log shorter, but the fan-out itself is just as wide — reach for it when a single run would otherwise accumulate tens of thousands of events, not to reduce concurrency.
