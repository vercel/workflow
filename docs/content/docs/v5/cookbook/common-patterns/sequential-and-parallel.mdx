...
<Callout type="warn">
  Every step in a `Promise.all` lands in the same run's event log, and replay
  cost grows with that log. Past roughly **100 steps in a single run**, a wider
  fan-out starts adding latency instead of removing it. Fan-outs approaching
  that size belong in [batches](/cookbook/common-patterns/batching) or
  [child workflows](/cookbook/advanced/child-workflows). See
  [How much work fits in one run](/docs/foundations/workflows-and-steps#how-much-work-fits-in-one-run).
</Callout>
...
- **Bound the parallelism**: `Promise.all` over 1,000 items will fan out 1,000 concurrent steps into one run. Both your downstream APIs and the run's own event log have a limit here: past roughly 100 steps in a run, the extra parallelism costs more than it saves. Batch the array into chunks (see [Batching](/cookbook/common-patterns/batching)), or hand each item to a [child workflow](/cookbook/advanced/child-workflows) so each log stays bounded.
