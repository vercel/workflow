...
- **One run would otherwise get too big**: A run's event log is capped, see e.g. [Vercel World limits](/worlds/vercel#per-run-event-limit). If you expect a run to accumulate tens of thousands of events, split it so each log stays short.
...
- **Watch the parent's log too**: Each child bounds its own log, but the parent still records the spawn and completion of every child. A parent that fans out to tens of thousands of children needs its own layer of children, not a wider loop.
