...
- **One run would otherwise get too big**: A run's event log is capped, see e.g. [Vercel World limits](/worlds/vercel#per-run-event-limit), and replay cost grows with it long before the cap. Split workflows to keep each event log short.
