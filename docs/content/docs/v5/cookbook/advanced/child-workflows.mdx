...
- **Watch the parent's log too**: Each child bounds its own log, but the parent still records the spawn and completion of every child. For very large orchestration, multiple layers of child runs may be useful.
