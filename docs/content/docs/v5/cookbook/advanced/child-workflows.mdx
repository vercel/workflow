...
- **One run would otherwise get too big**: A run's event log is capped — 25,000 events on the [Vercel World](/worlds/vercel#per-run-event-limit) — and replay cost grows with it long before the cap. Splitting is how you keep each log short.
...
<Callout type="warn">
  Split *well before* the ceiling rather than sizing a run to just fit under it.
  A successful step records about three events, retries and hooks add more, and
  a run that exceeds its limit fails with `MAX_EVENTS_EXCEEDED` and cannot be
  continued. As a working rule, once a workflow's step count scales with its
  input — or a single fan-out approaches ~100 steps — move the per-item work
  into children. See
...
- **Use chunked spawning for large batches**: Starting 500 children at once can create a large burst of work, and it puts 500 concurrent steps in the *parent's* event log — the same fan-out cost the ~100-step guidance is about. Break the work into chunks of 10–50.
- **Watch the parent's log too**: Splitting work into children bounds each child's log, but the parent still records the spawn and completion of every child. A parent that fans out tens of thousands of children needs its own layer of children, not a wider loop.
