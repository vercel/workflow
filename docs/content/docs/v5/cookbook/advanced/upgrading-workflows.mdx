...
1. **`deploymentId: "latest"` is the upgrade knob.** Without it, the spawn pins to the current deployment. With it, the new run resolves to whatever deployment is current when the runtime picks it up, so any shipped fix applies starting from that respawn. The respawned run is also stamped with the spec version of the deployment it lands on, capped at the spawning deployment's own, so it never claims runtime features the spawning SDK could not write. Both methods rely on this.
