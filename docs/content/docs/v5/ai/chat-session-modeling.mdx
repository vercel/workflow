...
type AttributedChatMessage = UIMessage<{
  userId: string;
  userName: string;
}>;
...
export async function POST(
  ...
  const attributedMessage: AttributedChatMessage = {
    id: messageId,
    role: "user",
    metadata: {
      userId: user.id,
  ...
  await saveChatMessage(runId, attributedMessage); // [!code highlight]
  await chatMessageHook.resume(runId, {
...
Make `saveChatMessage` idempotent by `messageId`. If the database write and Hook resume must be delivered together despite process failure, use an application-level transactional outbox; resuming a Hook does not persist an attributed `UIMessage` in your chat database automatically.
