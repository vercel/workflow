...
AI Gateway is the default in this guide, but `WorkflowAgent` also accepts AI SDK provider instances. For example, install the OpenAI provider package:
...
export async function chatWorkflow(messages: UIMessage[]) {
  ...
  const agent = new WorkflowAgent({ // [!code highlight]
    ...
    model: "xai/grok-4.6", // [!code highlight]
    instructions: FLIGHT_ASSISTANT_PROMPT,
    tools: flightBookingTools,
  });
...
<Callout type="info">
The example defaults to AI Gateway. If you use a provider package directly, import its provider and pass the model instance instead, for example `model: openai("gpt-5.6-sol")` from `@ai-sdk/openai`. The rest of the `WorkflowAgent` integration is unchanged.
</Callout>
