...
- **Fire-and-forget at completion.** If you call `abort()` (or `dispose` a hook, or kick off a `void sleep('1d')`) without a suspension point between that call and the workflow's return, the runtime logs a warning but does not flush the pending operation. Aborts that need to propagate must occur before the workflow's last suspension.
