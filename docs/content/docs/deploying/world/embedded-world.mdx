...
By default, the local world constructs the base URL from the detected port (e.g., `http://localhost:3000`). You can override this with the `WORKFLOW_EMBEDDED_BASE_URL` environment variable or programmatically:

**Environment variable:**

```bash
export WORKFLOW_EMBEDDED_BASE_URL=http://localhost:8080
```
...
const world = createEmbeddedWorld({ baseUrl: 'http://localhost:8080' });
```
...
- `args` - Optional configuration object:
  ...
  - `port` - Server port for queue transport (default: `3000` or `PORT` env var)
  - `baseUrl` - Base URL for queue transport (default: `http://localhost:{port}` or `WORKFLOW_EMBEDDED_BASE_URL` env var)
