...
Workflow handler functions on Vercel are not accessible from through public endpoints. During the build step, the Workflow DevKit registers each handler as only reachable by [Vercel Queue](https://vercel.com/docs/queues), by using the `experimentalTriggers` configuration in `.vc-config.json`:
...
<Callout>
  This configuration is managed entirely by the Workflow DevKit build step. You never need to write this yourself — it's documented here for transparency. For custom framework integrations, see [Framework Integrations — Security](/docs/how-it-works/framework-integrations#security).
</Callout>
