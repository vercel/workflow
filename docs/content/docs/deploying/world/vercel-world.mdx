...
Workflow handler functions on Vercel are not accessible through public endpoints. During the build step, the Workflow DevKit registers each handler as only reachable by [Vercel Queue](https://vercel.com/docs/queues), by using the `experimentalTriggers` configuration in `.vc-config.json`:
