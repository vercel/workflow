...
Workflow handler functions on Vercel are not accessible from the outside world. During the build step, the Workflow DevKit automatically configures each handler as a [Vercel Queue](https://vercel.com/docs/queues) consumer using `experimentalTriggers` in the function's `.vc-config.json`:
