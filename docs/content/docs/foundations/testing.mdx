...
Without the workflow compiler, the `"use workflow"` and `"use step"` directives are no-ops. Your workflow and step functions run as regular JavaScript functions, making them straightforward to unit test.
...
describe("handleUserSignup workflow", () => {
  it("should return the onboarded status", async () => {
    const result = await handleUserSignup("test@example.com"); // [!code highlight]
