---
title: Testing
description: Test workflows and steps in isolation or against a real workflow runtime using Vitest and the Vite plugin.
type: guide
summary: Learn how to unit test and integration test your workflows using Vitest.
prerequisites:
...
Testing is a critical part of building reliable workflows. Because workflows and steps are just functions annotated with directives, they can be tested like any other JavaScript function. Workflow DevKit also provides a [Vite plugin](/docs/getting-started/vite) that integrates with [Vitest](https://vitest.dev), enabling full integration tests against a real workflow runtime.

This guide covers two approaches:

1. **Unit testing** — Test workflows and steps as plain functions, without the workflow runtime
2. **Integration testing** — Test against a real workflow setup using the Vite plugin, which is essential for features like [hooks](/docs/foundations/hooks), webhooks, and [`sleep()`](/docs/api-reference/workflow/sleep)

## Unit Testing Workflows and Steps

Without the workflow compiler, the `"use workflow"` and `"use step"` directives are simply ignored — they're no-ops. This means your workflow and step functions run as regular JavaScript functions, making them straightforward to unit test.
...
You can test these functions directly with Vitest — no special configuration needed:

```typescript title="workflows/user-signup.test.ts" lineNumbers
import { describe, it, expect } from "vitest";
import { handleUserSignup, createUser } from "./user-signup";
...
describe("handleUserSignup workflow", () => {
  it("should return the onboarded status", async () => {
    const result = await handleUserSignup("test@example.com");
...
This approach is great for verifying business logic in isolation. Since directives are no-ops outside the workflow runtime, `sleep()` calls resolve immediately and steps execute synchronously in sequence.

<Callout type="info">
Unit tests are fast and don't require any infrastructure. Use them for validating the core logic of your steps and the orchestration flow of your workflows.
</Callout>
...
<Callout type="info">
We recommend using a separate Vitest configuration and a distinct file naming convention (e.g. `*.integration.test.ts`) to keep unit tests and integration tests separate. Unit tests can run with a standard Vitest config (without the workflow plugin), while integration tests use the config above.
</Callout>
...
The real power of integration testing comes when testing workflow-only features. Hooks and waits can be **resumed programmatically** using the [`workflow/api`](/docs/api-reference/workflow-api) functions, making it easy to simulate external events in your tests.

For example, given a workflow that waits for approval via a hook:
...
- [Hooks & Webhooks](/docs/foundations/hooks) — Learn about pausing and resuming workflows with external data
