...
2. **Integration testing** — Test against a real workflow setup using the Vite plugin, which is required for features like [hooks](/docs/foundations/hooks), webhooks, and [`sleep()`](/docs/api-reference/workflow/sleep).
...
This approach is ideal for verifying business logic in isolation. Since directives are no-ops outside the workflow runtime, `sleep()` calls resolve immediately and steps execute synchronously in sequence.
