...
You can dispose a hook early using the `dispose()` method. This releases the hook token so it can be reused by another workflow, even while the current workflow is still running. This is useful when:

- You want to release a custom token for another workflow to use
- You're done receiving events and want to clean up resources
- You're implementing a handoff pattern between workflows
...
export async function handoffWorkflow(channelId: string) {
  ...
  for await (const payload of hook) {
    ...
    if (payload.handoff) {
      // Release the token so another workflow can use it
      hook.dispose();
      break;
    }
  }
...
<Callout type="info">
After calling `dispose()`, the hook will no longer receive any events. The async iterator will stop yielding values.
</Callout>
