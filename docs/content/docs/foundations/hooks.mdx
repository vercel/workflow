...
Let's start with a simple example. Here's a workflow that creates a hook and waits for external data. We recommend using the `using` keyword which automatically disposes the hook when it goes out of scope:
...
export async function approvalWorkflow() {
  ...
  // Create a hook with `using` for automatic disposal
  ...
  // Hook is automatically disposed when the block/function exits // [!code highlight]
}
```
...
<Callout type="info">
Hooks implement the [TC39 Explicit Resource Management](https://github.com/tc39/proposal-explicit-resource-management) proposal, so they can be used with the `using` keyword for automatic cleanup. This is the recommended approach.
</Callout>
...
export async function slackChannelBot(channelId: string) {
  ...
  // Hook is automatically disposed when the function exits // [!code highlight]
}
...
export async function dataCollectionWorkflow() {
  ...
  using hook = createHook<{ value: number; done?: boolean }>(); // [!code highlight]
...
Hooks implement the [TC39 Explicit Resource Management](https://github.com/tc39/proposal-explicit-resource-management) proposal, which means you can use the `using` keyword for automatic disposal. This is the **recommended approach** as it ensures hooks are always cleaned up, even if an error occurs:
...
export async function handoffWorkflow(channelId: string) {
  ...
  {
    using hook = createHook<{ message: string; handoff?: boolean }>({ // [!code highlight]
    ...
    // Hook is automatically disposed when the block exits // [!code highlight]
  }

  // Continue with other work - token is now available for reuse
  console.log("Hook disposed, token is now available for reuse");
}
```

You can also manually dispose a hook using the `dispose()` method if you need more control:
...
<Callout type="info">
After disposal (automatic or manual), the hook will no longer receive any events. The async iterator will stop yielding values.
</Callout>
...
Here's a simple webhook that receives HTTP requests. Like hooks, we recommend using the `using` keyword for automatic disposal:
...
export async function webhookWorkflow() {
  ...
  // Webhook is automatically disposed when the function exits // [!code highlight]
}
```
...
export async function documentApprovalWorkflow(documentId: string) {
  ...
  using hook = approvalHook.create({ // [!code highlight]
...
Both hooks and webhooks support iteration, making them perfect for long-running event loops. Use the `using` keyword to ensure automatic cleanup:

{/* @skip-typecheck: incomplete code sample */}
```typescript
using hook = createHook<Event>(); // [!code highlight]
...
// Hook is automatically disposed when it goes out of scope // [!code highlight]
```
