...
Determinism in the workflow is required in order to resume to workflow from a suspension. Essentially, the workflow code get re-run multiple times during its lifecycle,
each time, using an event log to resume the workflow to the correct spot.

The sandboxed environment that worklfows are run in already ensure determinism. For instance, `Math.random` and `Date` constructors are fixed in workflow runs, so you are safe to use them and the framework ensures that the values don't cahange across replays.
...
By default, steps have a maximum of 3 retry attempts before it fails and propagates the error over to the workflow. Learn more about errors any retrying in the [Errors & Retrying](/docs/foundations/errors-and-retries) page.
...
But there is more than one way for a worklfow to suspend:
...
export async function documentReviewProcess(userId: string) {
  ...
  await sleep("1 month"); // Sleep will suspend witout consuming any resources [!code highlight]
...
The simplest form of workflow consists of a workflow function and one or more step functions.
...
While you can organize worklfow and step functions however you like, we find that larger projects benefit from some structure:
<Files>
