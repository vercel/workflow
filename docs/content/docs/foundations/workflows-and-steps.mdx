...
The sandboxed environment that workflows are run in already ensure determinism. For instance, `Math.random` and `Date` constructors are fixed in workflow runs, so you are safe to use them and the framework ensures that the values don't change across replays.
...
But there is more than one way for a workflow to suspend:
