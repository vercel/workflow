...
<Callout type="warn">
`returnValue` polls the child workflow every 1 second and holds a serverless worker alive for the entire duration of the child workflow. For long-running children, prefer a fire-and-forget pattern combined with [hooks](/docs/foundations/hooks) to receive a notification when the child completes.
</Callout>
...
By default, `start()` schedules the child workflow on the **same deployment** as the caller. This is usually what you want for parent-child workflows within the same request. However, for self-chaining workflows that run over long periods, you may want the next run to use the **latest deployment** so it picks up new code:
...
<Callout type="warn">
When using `deploymentId: "latest"`, type safety is not guaranteed across deployments. If the workflow's input arguments or return value types change between deployments, the daisy-chained workflow may receive unexpected data. It is your responsibility to ensure workflows remain backwards-compatible when using this pattern.
</Callout>
