...
export async function parentWorkflow(inputValue: number) {
  ...
  // childRun.runId is available immediately
  return { childRunId: childRun.runId };
}
```

When `start()` is called inside a workflow function, it automatically executes through an internal step to maintain deterministic replay. The returned object contains a `runId` property that you can use to track the child workflow.

<Callout type="info">
Inside workflow functions, `start()` returns an object with only the `runId` property (not a full `Run` object). To get full `Run` capabilities like `.returnValue` or `.cancel()`, use `getRun(runId)` inside a step function.
</Callout>
