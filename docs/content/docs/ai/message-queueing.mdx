...
interface PrepareStepInfo {
  model: string | (() => Promise<LanguageModelV3>);  // Current model
  stepNumber: number;                                // 0-indexed step count
  steps: StepResult[];                               // Previous step results
  messages: LanguageModelV3Prompt;                   // Messages to be sent
}

interface PrepareStepResult {
  model?: string | (() => Promise<LanguageModelV3>); // Override model
...
<Callout type="info">
The `prepareStep` callback receives messages in `LanguageModelV3Prompt` format (with content arrays), which is the internal format used by the AI SDK.
</Callout>
