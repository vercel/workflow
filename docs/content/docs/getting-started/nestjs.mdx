...
Add the workflow build scripts to your `package.json`:

```json title="package.json" lineNumbers
{
  "scripts": {
    "workflow:build": "workflow-nest build",
    "prebuild": "npm run workflow:build",
    "build": "nest build",
    "prestart:dev": "npm run workflow:build",
    "start:dev": "nest start --watch"
  }
}
```

The `workflow-nest build` command discovers your workflow files and generates:
- Workflow bundles in `.workflow/` directory
- A `client.js` file with workflow metadata for use with `start()`

<Callout>
Add `.workflow` to your `.gitignore` as it contains generated files that shouldn't be committed.
</Callout>
...
First, import the workflow metadata from the generated client file:
...
import { handleUserSignup } from '../.workflow/client.js'; // [!code highlight]
...
<Callout>
Note that we import `handleUserSignup` from `.workflow/client.js`, not from the workflow file directly. The generated client file contains workflow metadata objects with `workflowId` attached, which is required for `start()` to work correctly.
</Callout>
...
Build the workflow bundles first:
...
WorkflowModule.forRoot({
  ...
  // Output directory for generated bundles (default: '.workflow')
  outDir: '.workflow',
...
The `@workflow/nest` package includes a CLI for building workflow bundles:
...
npx @workflow/nest build --dirs src/workflows --out .workflow

# Watch mode for development
npx @workflow/nest dev
```
