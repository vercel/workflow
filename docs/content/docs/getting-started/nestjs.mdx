...
WorkflowModule.forRoot({
  ...
  // Inline source maps on step and workflow bundles (default: 'inline').
  // Set to 'disabled' (or false) to omit source maps for smaller bundles,
  // at the cost of workflow VM stack traces pointing at generated code
