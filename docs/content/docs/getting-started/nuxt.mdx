...
Add `workflow/nuxt` to your `nuxt.config.ts`. This automatically configures the `workflow/nitro` module and enables usage of the `"use workflow"` and `"use step"` directives.
