...
```typescript lineNumbers
await world.streams.writeToStream("default", runId, chunk); // [!code highlight]
```
...
```typescript lineNumbers
const readable = await world.streams.readFromStream("default"); // [!code highlight]
```
...
```typescript lineNumbers
await world.streams.closeStream("default", runId); // [!code highlight]
```
...
```typescript lineNumbers
const streamNames = await world.streams.listStreamsByRunId(runId); // [!code highlight]
```
...
export async function GET(req: Request) {
  ...
  const world = getWorld();
  const readable = await world.streams.readFromStream(streamName); // [!code highlight]
