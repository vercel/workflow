...
There is no `cancel()` method on `world.runs`. The previous `world.runs.cancel()` method was removed — to cancel a run, create a `run_cancelled` event via `world.events.create()` instead (see [Events](/docs/api-reference/workflow-api/world/events)).
