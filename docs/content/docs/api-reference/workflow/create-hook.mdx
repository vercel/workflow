...
We recommend using the `using` keyword for automatic disposal when the hook goes out of scope:
...
export async function hookWorkflow() {
  ...
  // Hook is automatically disposed when it goes out of scope
}
```
...
When creating a hook, you can specify a payload type for automatic type safety. Use the `using` keyword for automatic disposal:
...
export async function collectHookWorkflow() {
  ...
  using hook = createHook<{ message: string; done?: boolean }>(); // [!code highlight]
