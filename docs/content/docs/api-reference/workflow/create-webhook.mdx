...
We recommend using the `using` keyword for automatic disposal when the webhook goes out of scope:
...
export async function webhookWorkflow() {
  ...
  // Webhook is automatically disposed when it goes out of scope
}
```
...
Create a webhook that receives HTTP requests and logs the request details. Use the `using` keyword for automatic disposal:
...
export async function basicWebhookWorkflow() {
  ...
  // Webhook is automatically disposed when the function exits
}
```
