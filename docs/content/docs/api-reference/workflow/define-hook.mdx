...
Combine compile-time safety with runtime validation by providing a schema.
...
const approvalHook = defineHook(schema);

export async function POST(request: Request) {
  const { token, approved, comment } = await request.json();
  await approvalHook.resume(token, { approved, comment });
  // If validation fails, a Zod error is thrown before resuming the hook.
