...
| `workflows.sourcemap` | `boolean \| 'inline' \| 'disabled'` | `'inline'` | Controls inline source maps on the step and workflow bundles. Set to `'disabled'` (or `false`) to omit them for smaller bundles, at the cost of workflow VM stack traces pointing at generated code instead of your source files. |
