...
<TSDoc
definition={`
import type { StreamTextOnChunkCallback } from "@workflow/ai/agent";
export default StreamTextOnChunkCallback;`}
/>
...
- The `stream()` method returns `{ messages, steps, experimental_output }` containing the full conversation history, step details, and optional structured output
