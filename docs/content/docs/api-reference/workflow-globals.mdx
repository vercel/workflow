...
| [`Date`](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Date) / `Date.now()` / `new Date()` | Returns the workflow's logical clock: the run's creation time until the first step result, hook payload, hook registration or wait completion reaches the workflow, then the time that event was recorded, advancing as each later one is delivered. Identical on every replay of the same log |
