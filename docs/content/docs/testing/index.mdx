...
<Callout type="warn">
Inside integration tests, which run the full workflow runtime, `vi.mock()` and related calls do not work for first-party code imported by your steps. Step dependencies are inlined into the compiled bundle, bypassing Vitest's module system. To test steps with mocked dependencies, use [unit tests](#unit-testing-steps) instead. Consider dependency injection or environment variable-based conditional logic for controlling behavior in integration tests.
</Callout>
