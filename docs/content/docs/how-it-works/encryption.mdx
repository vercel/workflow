...
For example, the [Vercel World](/docs/deploying/world/vercel-world) uses a two-level key hierarchy: each deployment receives a unique 32-byte random key, and per-run keys are derived from it using [HKDF-SHA256](https://datatracker.ietf.org/doc/html/rfc5869) with the project ID and run ID as context. This ensures that a key derived for one run cannot be used to decrypt data from another run.
