...
<Callout>
This guide explains the design rationale behind the Workflow DevKit's use of JavaScript directives. It's not required reading to use workflows—check out the [getting started](/docs/getting-started) guides for your framework instead.
</Callout>

When we introduced the Workflow Development Kit, we chose JavaScript directives (`"use workflow"` and `"use step"`) as the mechanism to mark workflow code. This page explains why we made that choice and the alternatives we considered.

To understand why we needed directives, let's first understand what workflows and steps are in the Workflow DevKit.
...
**Workflow functions** are deterministic orchestrators that coordinate steps. Given the same input, they must produce the same output. They run in a sandboxed environment without direct Node.js access:
...
export async function onboardUser(userId: string) {
  ...
  const welcomeEmail = await sendWelcomeEmail(user); // Calls another step

  return `Onboarded ${user.name}!`;
}
```

**The key insight:** When a workflow calls a step like `await fetchUserData(userId)`, the semantics differ significantly from normal async/await:

- **If the step was already executed:** The workflow retrieves the cached result from the event log and returns immediately
- **If the step hasn't run yet:** The workflow suspends, enqueues the step for background execution, and will resume later when the step completes

This creates a fundamental drift from normal JavaScript `async/await` semantics. The same syntax (`await`) is used, but the execution model is completely different—workflow functions replay multiple times, and step calls don't actually execute code but instead interact with the durable execution engine.
...
- `"use strict"` changes language rules to make the runtime faster, safer, and more predictable.
- `"use client"` and `"use server"` define an explicit boundary of "where" code gets executed.
- `"use workflow"` defines **execution semantics** — deterministic, resumable, sandboxed.
...
In addition to being important to the compiler, `"use workflow"` explicitly signals to the developer ([and IDE](/docs/getting-started/next#setup-typescript-intellisense-optional)) that you are entering a different execution mode.
...
export const myWorkflow = workflow(function*() {
  ...
  // Combining with Promise.all required awkward syntax // [!code highlight]
  const [orders, payments] = yield* run(() => // [!code highlight]
    Promise.all([ // [!code highlight]
      yield* run(() => fetchOrders(user.id)), // [!code highlight]
      yield* run(() => fetchPayments(user.id)) // [!code highlight]
    ]) // [!code highlight]
  ); // [!code highlight]
...
With this approach, any function in the `workflows/` directory would be transformed as a workflow, and any function in `steps/` would be a step. No directives needed—just file location.
...
Different frameworks and developers have strong opinions about project structure. Forcing a specific directory layout often caused conflicts across various conventions.
...
**Decorators are non-standard and class-focused**

TypeScript decorators are non-standard syntax that currently only work with classes. A class decorator approach would look like this:
...
As the JavaScript ecosystem has moved toward functional programming—exemplified by React's shift from class components to functions and hooks—requiring developers to use classes felt like a step backward and also didn't match our own personal taste as authors of the DevKit.

Moreover, this approach still suffered from the aforementioned drawbacks of any runtime-only approach:

**The core problem is still: Runtime semantics**

Decorators and wrappers operate at runtime—they can intercept function calls but can't change how the code inside executes. The function body still runs as normal JavaScript with full Node.js access.

A decorator or wrapper can *describe* behavior but not *redefine* it.

```typescript lineNumbers
@Workflow()
static async processOrder(orderId: string) {
  // This code runs in Node.js with full runtime access // [!code highlight]
  const now = Date.now(); // Non-deterministic, but nothing stops it // [!code highlight]
  const file = fs.readFileSync('./config.json'); // Direct file access // [!code highlight]
  const order = await fetchOrder(orderId);

  return { orderId, now, config: file };
}
```

Without compile-time transformation:

- The compiler treats decorated/wrapped functions like any other code
- We can't statically analyze the workflow structure
- We can't prevent non-deterministic APIs
- We can't enforce sandboxing at build time

### The Macro Wrapper

We also explored a function wrapper macro approach—still using a compiler but "presenting" it as a wrapper function to avoid the directive:
...
The compiler could transform that code to be equivalent to:
...
The benefit here is that the macro could enforce types and provide "Go To Definition" or other LSP features out of the box.

However, **the core problem remains: Workflows aren't runtime values**

The fundamental issue is that the wrapper approach makes workflows appear to be **first-class, runtime values** when they're actually **compile-time declarations**. This mismatch between syntax and semantics creates numerous failure modes.
...
function withRetry(fn: Function) {
  return useWorkflow(async (...args) => { // [!code highlight]
    try {
      return await fn(...args); // [!code highlight]
    } catch (error) {
...
The developer writing `processOrder` has no visible signal that they're in a deterministic, sandboxed environment. It's also ambiguous whether the retry logic executes inside the workflow (deterministic) or outside (non-deterministic)—and the actual behavior likely doesn't match developer intuition.

---

## Why Directives Solve These Problems
...
- Infact, Workflow DevKit will throw an error that links to this error page: [Node.js module in workflow](/docs/errors/node-js-module-in-workflow)
...
We're open to working with TC39 and the broader ecosystem on defining a standard for pragma-like syntax or macro annotations that can express execution semantics formally.
