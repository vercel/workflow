...
<Callout>
See the [Macro Wrapper](/docs/how-it-works/why-directives#the-macro-wrapper) section below for a deeper dive into why this approach breaks down with concrete examples.
</Callout>
...
class MyWorkflow {
  @workflow() // [!code highlight]
  static async function processOrder2(orderId: string) {
    const order = await fetchOrder(orderId);
    return { orderId };
  }
