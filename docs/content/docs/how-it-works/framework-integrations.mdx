...
On Vercel, workflow handler functions are completely air-gapped from the internet using [consumer function security](https://vercel.com/docs/queues/concepts#consumer-function-security), the same mechanism that secures [Vercel Queues](https://vercel.com/docs/queues) consumers.

During the build step, the Workflow DevKit automatically configures each handler as a queue consumer by writing `experimentalTriggers` to the function's `.vc-config.json`:

```json title=".vc-config.json (generated by Workflow DevKit)"
{
...
- The handler functions have **no public URL** and cannot be reached from the internet
- Only Vercel's internal queue infrastructure can invoke them
...
- You don't need to add any authentication or authorization logic to your handlers
...
See the [Vercel World — Consumer function security](/docs/deploying/world/vercel-world#consumer-function-security) documentation for the full configuration reference.
