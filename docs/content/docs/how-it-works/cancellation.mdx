...
Normally, the invocations queue is only processed when the workflow suspends (throws `WorkflowSuspension`). If the workflow completes (or fails) without suspending, any remaining queue items are dropped with a warning.

The runtime processes all pending invocations queue items when the workflow completes or fails — not just on suspension. This is a general improvement that applies to all queue item types (steps, hooks, waits, abort signals), not just abort. For example, if a step is created as the run is completing, it should still be enqueued for execution.
