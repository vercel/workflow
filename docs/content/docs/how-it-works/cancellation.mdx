...
- **On replay**: The event consumer sees the `hook_received` event and reconstructs `signal.aborted` as `true`
...
stepFunction(controller.signal)
        │
        ├─→ Signal serialized as { streamName, aborted }
...
controller.abort()
        │
        ├─→ Local signal marked as aborted (synchronous)
        ├─→ Hook resumed → hook_received event in event log
        └─→ Cancellation packet written to stream
                │
...
Replay starts → events loaded
        ...
        ├─→ hook_received event consumed → signal.aborted = true
        └─→ Workflow code sees signal.aborted === true deterministically
```
...
**Passed as workflow input** — When an `AbortController` or `AbortSignal` is passed to `start()` from external code, the **external reducer** handles it at serialization time:

1. Creates the backing stream name
...
4. Serializes the reference as `{ streamName, aborted }`

When the workflow deserializes the input, the workflow reviver creates the hook — same as the "returned from a step" case. If the external code calls `abort()` while the process is still alive (within the `waitUntil` window), the stream packet arrives in the workflow, and the workflow can resume the hook to record it in the event log.
...
1. The local signal state is updated synchronously
2. The internal hook is marked for resumption in the invocations queue (same pattern as `hook.dispose()`)
3. On suspension, the suspension handler processes the abort:
   - Creates a `hook_received` event
   - Writes the cancellation packet to the stream
