...
**Fix**: Normalize legacy worlds at runtime by adapting the old top-level stream methods (`writeToStream`, `writeToStreamMulti`, `closeStream`, `readFromStream`, `listStreamsByRunId`) into the current `world.streams.*` shape when an adapter has not upgraded yet.
