...
When multiple background steps complete near-simultaneously, the last ~2 handlers may both see all steps done and replay inline concurrently. The event-sourced architecture ensures safe convergence:
...
The V2 inline execution loop creates **out-of-order replay visibility**: a handler can see events in the log that were created by a different concurrent handler for subscriptions the current replay hasn't reached yet. For example, handler A completes batch 1 inline and creates `step_created` events for batch 2, while handler B (from a batch 1 background step that just completed) replays and encounters batch 2's events before reaching the code that creates those subscriptions. This triggers "Unconsumed event in event log" errors.

This is a fundamental consequence of allowing concurrent handlers to share an event log without event scoping. Server-side idempotency (409 on duplicates) prevents data corruption, but the event consumer sees events without matching subscribers and errors. A long-term fix would be server-side event scoping (each invocation only sees events it created via a `createdBy` field filter), but this requires schema changes in workflow-server.

**Fix**: The `EventsConsumer`'s `onUnconsumedEvent` callback in `packages/core/src/workflow.ts` uses lazily-built `Set<string>` indexes (initialized on first unconsumed event) for O(1) correlationId lookup:

- **Step lifecycle events** (`step_created`, `step_started`, `step_completed`, `step_failed`, `step_retrying`): skipped if the correlationId appears in a `step_created` event in the log — confirming the step was legitimately created by a handler.
- **Hook lifecycle events** (`hook_created`, `hook_received`, `hook_conflict`): skipped if the correlationId appears in a `hook_created` event.
- **Duplicate `wait_completed`**: skipped only if an earlier `wait_completed` for the same correlationId already exists in the log (the workflow already consumed the first one).
- **All other unconsumed events**: still error, preserving corruption detection for orphaned events with unknown correlationIds.

The `EventsConsumer` was extended to support a `boolean` return from `onUnconsumedEvent`: `true` advances past the event, `false`/`undefined` triggers the error.
...
### CJS Combined Bundle Import in world-testing

The V2 combined bundle is output in CJS format by the standalone builder (step code has CJS Node.js dependencies that require `require()`). The `world-testing` test server imported from `flow.mjs`, but Node.js treats `.mjs` files as ESM regardless of content, causing `SyntaxError: does not provide an export named 'POST'`.

**Fix** (two parts):
1. The server uses `createRequire` to import from `flow.js` (CJS extension) instead of `flow.mjs`, so Node.js applies CJS semantics correctly.
2. The standalone builder's default config (`workflow-config.ts`) was updated to output `step.js` and `flow.js` instead of `step.mjs` and `flow.mjs`, since the standalone builder always uses CJS format. Without this, the builder produced `flow.mjs` containing CJS code — which worked locally when a stale `flow.js` existed from a previous build, but failed on CI (clean checkout) because no `flow.js` was ever generated.
