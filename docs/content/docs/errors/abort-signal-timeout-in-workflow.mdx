---
title: AbortSignal.timeout() in Workflow
...
declare function fetchData(signal: AbortSignal): Promise<unknown>; // @setup
...
The `sleep()` + `AbortController` pattern is the durable equivalent of `AbortSignal.timeout()`. The sleep is recorded in the event log, so it replays deterministically.
