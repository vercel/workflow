...
This error occurs when the Workflow runtime tries to execute a workflow function that is not registered in the current deployment. When this happens, the run fails with a `FUNCTION_NOT_REGISTERED` error code.
