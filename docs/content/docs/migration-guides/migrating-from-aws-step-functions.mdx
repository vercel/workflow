...
// Before (ASL) — excerpt from a state machine's "States" block
"LoadOrder": {
  "Type": "Task",
...
// Before (ASL) — excerpt; paired with a consumer that calls SendTaskSuccess
"WaitForApproval": {
  "Type": "Task",
...
```json
// Before (ASL) — excerpt; "Approved" and "Rejected" are sibling states in "States"
"CheckApproval": {
  "Type": "Choice",
  "Choices": [
