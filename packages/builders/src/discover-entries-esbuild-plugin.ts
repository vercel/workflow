...
// Pattern to detect generated workflow route files that should be excluded from discovery
...
// Pattern to detect @workflow package paths that should be excluded from discovery
    ...
    setup(build) {
      ...
      build.onLoad({ filter: jsTsRegex }, async (args) => {
        try {
          ...
          if (generatedWorkflowPathPattern.test(args.path)) {
