...
export interface EventResult {
   ...
   *   `WorldCapabilities.slotEventIds`) whose committed slot came out higher
   *   than the one {@link CreateEventParams.eventCount} asked for: the events
   *   occupying the slots that were skipped over, in slot order. This is the
   *   "report" half of bump-and-report — the write succeeded, and these are
   *   the events the writer had not seen when it decided to make it.
   */
  events?: Event[];
