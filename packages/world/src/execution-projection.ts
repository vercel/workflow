  ...
  for (const event of snapshot.events) {
    ...
    if (event.eventType === 'run_created') {
      run = {
        ...
        attributes: event.eventData.attributes ?? {},
        createdAt: at,
        updatedAt: at,
      };
    } else {
      ...
      switch (event.eventType) {
        ...
        case 'attr_set':
          ...
          for (const { key, value } of event.eventData.changes) {
            if (value === null) delete run.attributes[key];
        ...
        case 'step_retrying': {
          ...
          if (event.eventType === 'step_started') {
            ...
            next.error = event.eventData.error;
            next.retryAfter =
              event.eventData.retryAfter === undefined
                ? undefined
                : new Date(event.eventData.retryAfter);
          }
    ...
    result = {
      ...result,
      run,
      ...(steps.has(correlationId!) ? { step: steps.get(correlationId!) } : {}),
      ...(hooks.has(correlationId!) ? { hook: hooks.get(correlationId!) } : {}),
      ...(waits.has(correlationId!) ? { wait: waits.get(correlationId!) } : {}),
    } as EventResult;
