...
export default {
  ...
  async setup(nitro: Nitro) {
    ...
    nitro.hooks.hook('rollup:before', (_nitro: Nitro, config: RollupConfig) => {
      ...
      // `require('node:http2')` resolves in the ESM server bundle) now lives in
      // @workflow/world-vercel itself, which installs the shim at module load
      // and falls back to HTTP/1.1 if node:http2 is unreachable. No per-bundler
      // banner is needed here anymore.
    });
