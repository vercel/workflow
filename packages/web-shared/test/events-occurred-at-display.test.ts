...
describe('event occurredAt display', () => {
  it('uses occurrence time as the created timestamp in detail panel rows by default', () => {
    ...
    // The first row anchors with the absolute occurrence time (12:34:56),
    // preferred over the createdAt wall-clock (12:34:57).
    expect(markup).toContain('12:34:56');
    expect(markup).not.toContain('12:34:57');
    // Subsequent rows show the inter-event gap derived from occurrence times
    // (12:35:01 − 12:34:56 = 5s) rather than a bare clock time.
    expect(markup).toContain('+5s');
    expect(markup).not.toContain('12:35:01');
