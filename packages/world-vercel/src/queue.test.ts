...
// registers and that getTraceCollectionHeaders reads). Lets tests simulate an
// incoming `_vercel_tracing` cookie on the request that triggers the enqueue.
...
describe('createQueue', () => {
  ...
  describe('createQueueHandler()', () => {
    ...
    it('should propagate x-vercel-tracing on delayed re-enqueue', async () => {
      mockSend.mockResolvedValue({ messageId: 'new-msg-123' });
      // The delivery request carried the VQS-forwarded cookie; the re-enqueue
      // runs inside that request context, so it re-attaches the trace header.
      setRequestContextCookie('_vercel_tracing=jwt.trace.token');
      const handler = setupHandler({ timeoutSeconds: 300 });
