...
export function createWorld(config?: APIConfig): World {
  ...
  return {
    ...
    standaloneStreams: streamer.standaloneStreams,
    createRunId,
    createStandaloneStreamId,
    standaloneStreamIdFor: async ({ name, region }) => {
      if (!projectId) {
        throw new Error(
          'standaloneStreamIdFor requires VERCEL_PROJECT_ID or projectConfig.projectId'
        );
      }
      const environment = resolveClientEnvironment(config);
      if (!environment) {
        throw new Error(
          'standaloneStreamIdFor requires VERCEL_TARGET_ENV/VERCEL_ENV or projectConfig.environment'
        );
      }
      return standaloneStreamIdFor({ projectId, environment, name, region });
    },
    describeRun,
