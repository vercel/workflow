...
const GlobalStreamEncryptionSchema = z
  .object({
...
const GlobalStreamInfoResponseSchema = StreamInfoResponseSchema.extend({
  ...
  encryption: GlobalStreamEncryptionSchema.nullable(),
...
export function encodeGlobalStreamEncryption(
  encryption: GlobalStreamWriteOptions['encryption']
): string {
...
function createGlobalStreamer(
  ...
  const put = async (
    ...
    if (options) {
      headers.set(
        'X-Stream-Encryption',
        encodeGlobalStreamEncryption(options.encryption)
      );
    }
