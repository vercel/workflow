...
import {
  ...
  type StandaloneStreamer,
  type StandaloneStreamInfoResponse,
  type StandaloneStreamWriteOptions,
...
function getStandaloneStreamUrl(id: string, httpConfig: HttpConfig) {
...
async function createStandaloneStreamReadError(
  response: Response
): Promise<Error> {
  if (response.status === 410) {
    let message = 'Standalone stream data is no longer available';
    try {
  ...
  return errorForResponse(
    response.status,
    `Failed to fetch standalone stream: ${response.status}`
  );
}

async function createStandaloneStreamWriteError(
  url: URL,
  response: Response
): Promise<Error> {
  const fallback = `Standalone stream write failed: HTTP ${response.status}`;
  try {
    const body = (await response.json()) as {
      error?: unknown;
      message?: unknown;
...
const StandaloneStreamEncryptionSchema = z
  .object({
...
const StandaloneStreamInfoResponseSchema = StreamInfoResponseSchema.extend({
  earliestIndex: z.number(),
  encryption: StandaloneStreamEncryptionSchema.nullable(),
...
export function createStreamer(
  ...
  return {
    ...
    standaloneStreams: createStandaloneStreamer(config),
  };
}

export function encodeStandaloneStreamEncryption(
  encryption: StandaloneStreamWriteOptions['encryption']
): string {
...
function createStandaloneStreamer(
  config?: APIConfig
): StandaloneStreamer['standaloneStreams'] {
  const put = async (
    id: string,
    body: string | Uint8Array | undefined,
    options: StandaloneStreamWriteOptions | undefined,
    multi = false,
    done = false
  ) => {
    ...
    if (options) {
      headers.set(
        'X-Stream-Encryption',
        encodeStandaloneStreamEncryption(options.encryption)
      );
    }
    if (multi) headers.set('X-Stream-Multi', 'true');
    if (done) headers.set('X-Stream-Done', 'true');
    const url = getStandaloneStreamUrl(id, http);
    const response = await instrumentedFetch({
      method: 'PUT',
      url: url.toString(),
      body,
      ...
      logLabel: url.pathname,
      buildError: options
        ? (res) => createStandaloneStreamWriteError(url, res)
        : async (res) =>
            createStreamRequestError('close', url, res, await res.text()),
    });
    await response.text();
  ...
  return {
    ...
    async get(id, startIndex) {
      ...
      const response = await instrumentedFetch({
        ...
        timeoutMs: null,
        logLabel: url.pathname,
        buildError: createStandaloneStreamReadError,
      });
      if (!response.body)
        throw new Error('No response body for standalone stream');
      return response.body as ReadableStream<Uint8Array>;
    },
    ...
    getInfo(id): Promise<StandaloneStreamInfoResponse> {
      return makeRequest({
        endpoint: `/v3/streams/${encodeURIComponent(id)}/info`,
        config,
        schema: StandaloneStreamInfoResponseSchema,
      });
    },
    async delete(id) {
      const http = await getHttpConfig(config);
      ...
      const response = await instrumentedFetch({
        ...
        buildError: async (res) =>
          errorForResponse(
            res.status,
            `Standalone stream delete failed: HTTP ${res.status}: ${await res.text()}`
          ),
      });
      await response.text();
    },
