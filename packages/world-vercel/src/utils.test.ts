...
describe('makeRequest transport errors', () => {
  ...
  it('maps a bare `TypeError: fetch failed` to a retryable TRANSPORT error', async () => {
    ...
    expect(rejection.cause).toBe(fetchErr);
    // That code is what both retry layers key on: queue redelivery in
    // `@workflow/core` (isRetryableWorldError, which also classifies the
    // terminal failure as WORLD_CONTRACT_ERROR rather than USER_ERROR) and
    // the in-process event POST retry here.
    expect(isRetryableEventPostError(rejection)).toBe(true);
  });
