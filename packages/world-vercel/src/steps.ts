...
export async function listWorkflowRunSteps(
  ...
  const endpoint = `/v3/runs/${encodeURIComponent(runId)}/steps${queryString ? `?${queryString}` : ''}`;
...
export async function createStep(
  ...
  const step = await makeRequest({
    endpoint: `/v3/runs/${encodeURIComponent(runId)}/steps`,
    options: { method: 'POST' },
    data,
    config,
...
export async function updateStep(
  ...
  const step = await makeRequest({
    endpoint: `/v3/runs/${encodeURIComponent(runId)}/steps/${encodeURIComponent(stepId)}`,
    options: { method: 'PUT' },
    data: serialized,
    config,
    schema: StepWireSchema,
...
export async function getStep(
  ...
  const endpoint = runId
    ? `/v3/runs/${encodeURIComponent(runId)}/steps/${encodeURIComponent(stepId)}${queryString ? `?${queryString}` : ''}`
    : `/v3/steps/${encodeURIComponent(stepId)}${queryString ? `?${queryString}` : ''}`;
