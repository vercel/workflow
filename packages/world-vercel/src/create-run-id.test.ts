...
describe('createStandaloneStreamId', () => {
  it('uses the standalone prefix and the region-tagged run ID encoding', () => {
    process.env.VERCEL_REGION = 'sfo1';
    const id = createStandaloneStreamId();
    expect(id).toMatch(/^stnd_/);
    expect(decode(id.slice('stnd_'.length)).region).toBe('sfo1');
  });
});

describe('standaloneStreamIdFor', () => {
  it('is stable, tenant-scoped, region-tagged, and version 2', () => {
    const first = standaloneStreamIdFor({
      projectId: 'prj_one',
      environment: 'production',
      name: 'chat',
      region: 'sfo1',
    });
    const second = standaloneStreamIdFor({
      projectId: 'prj_one',
      environment: 'production',
      name: 'chat',
      region: 'sfo1',
    ...
    expect(first).toMatch(/^stnd_[0-7][0-9A-HJKMNP-TV-Z]{25}$/);
