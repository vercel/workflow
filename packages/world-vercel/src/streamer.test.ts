...
describe('globalStreams transport', () => {
  ...
  it('sends the canonical envelope on writes but not close', async () => {
    const fetchSpy = vi
      .spyOn(globalThis, 'fetch')
    ...
    const encodedEnvelope = writeHeaders.get('X-Stream-Encryption');
    expect(encodedEnvelope).not.toBeNull();
    expect(Buffer.from(encodedEnvelope ?? '', 'base64url').toString()).toBe(
