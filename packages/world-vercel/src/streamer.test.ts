...
describe('standaloneStreams transport', () => {
  async function getStreamer() {
    const { createStreamer } = await import('./streamer.js');
    return createStreamer();
  }
  ...
  it('sends the canonical encryption on writes but not close', async () => {
    ...
    await streamer.standaloneStreams.write('stnd_abc', 'chunk', options);
    await streamer.standaloneStreams.close('stnd_abc');
    ...
    for (const call of fetchSpy.mock.calls) {
      expect(new URL(call[0] as string).pathname).toBe('/v3/streams/stnd_abc');
    }
    expect(
      new Headers((fetchSpy.mock.calls[1][1] as RequestInit).headers).get(
        'X-Stream-Done'
  ...
  it('preserves the encryption-mismatch code on write conflicts', async () => {
    ...
    const streamer = await getStreamer();
    await expect(
      streamer.standaloneStreams.write('stnd_abc', 'chunk', {
        encryption: {
          v: 1,
          s: 'dpl',
          d: 'dpl_anchor',
  ...
  it('maps every global read 410 to terminal stream expiry', async () => {
    ...
    const streamer = await getStreamer();
    await expect(
      streamer.standaloneStreams.get('stnd_abc')
  ...
  it('preserves delete status errors', async () => {
    ...
    const streamer = await getStreamer();
    await expect(
      streamer.standaloneStreams.delete('stnd_abc')
  ...
  it('frames multi writes and reads from the v3 route', async () => {
    ...
    await streamer.standaloneStreams.writeMulti(
      'stnd_abc',
      ['a', 'b'],
      options
    );
    ...
    await streamer.standaloneStreams.get('stnd_abc', 7).catch(() => {});
    expect(
    ...
    expect(url.pathname).toBe('/v3/streams/stnd_abc');
