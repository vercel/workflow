...
interface WorkflowVmPool {
  entries: WorkflowVmCacheEntry[];
}

const globalSymbols: typeof globalThis & {
  [WorkflowVmCacheSymbol]?: Map<string, WorkflowVmPool>;
} = globalThis;
...
export async function runWorkflow(
  ...
  return trace(`workflow.run ${workflowRun.workflowName}`, async (span) => {
    ...
    let workflowVmPool = workflowVmCache.get(workflowCacheKey);
    if (!workflowVmPool) {
      workflowVmPool = { entries: [] };
      workflowVmCache.set(workflowCacheKey, workflowVmPool);
    }

    let cachedVm = workflowVmPool.entries.find((entry) => !entry.inUse);
    ...
    try {
      ...
      if (!cachedVm) {
        // Initialize workflow code once per workflow VM. The loaded module code
        // captures the workflow context object, so each VM is leased to one run at a time.
        ...
        const filename =
          parsedName?.moduleSpecifier || workflowRun.workflowName;
        ...
        workflowVmPool.entries.push(cachedVm);
      }

      const cachedWorkflowFn = cachedVm.workflowFns.get(
