...
function makeWorldWithScriptedStreams(
  ...
  const world = {
    ...
    ...(getInfo ? { getStreamInfo: vi.fn(getInfo) } : {}),
  } as unknown as World;
  return { world, calls };
}
...
describe('createReconnectingFramedStream', () => {
  ...
  it('trusts EOF when stream metadata is unavailable', async () => {
    // A transient getStreamInfo failure must not turn a healthy completion
    // into an error or a reconnect loop — fall back to the legacy
    // trust-the-EOF behavior.
    const { world, calls } = makeWorldWithScriptedStreams(
      {
        0: () =>
          scriptedStream([
