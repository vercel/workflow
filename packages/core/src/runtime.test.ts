...
describe('workflowEntrypoint replay guards', () => {
  ...
  it('re-routes a misrouted lazy hook resume with its payload intact', async () => {
    ...
    // setup idempotently ensures the event before it can reconstruct the run
    // and check deployment affinity. The re-routed message must still carry
    // `hookInput` in case that setup request fails before the write lands.
    const workflowRun = await misroutedRun();
    const queueCalls: QueueCall[] = [];
    const hookInput = {
