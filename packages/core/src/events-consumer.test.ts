...
describe('EventsConsumer', () => {
  ...
  describe('duplicate event classes', () => {
    ...
    it('skips a duplicate wait_completed ahead of the divergence report', async () => {
      // Without the class check this would sit at the cursor waiting for a
      // consumer that can never come, and be reported as divergence.
      const corr = 'wait_A';
      const events = [
    ...
    it('still reports an unconsumed event for a correlation id the log has nothing for', async () => {
      const events = [
        ...
        // A different entity that no callback ever claims, and whose class no
        // consumption in this walk recorded, so nothing suppresses the report.
    ...
    it('does not track hook deliveries, whose consumers subscribe lazily', async () => {
      ...
      // deferred-check path rather than being skipped.
      const corr = 'hook_A';
      const events = [
    ...
    it('leaves a duplicate run_cancelled to the deferred check', async () => {
