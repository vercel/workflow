...
describe('workflow arguments', () => {
  ...
  it('round-trips and writes a standalone stream descriptor without run lookup', async () => {
    ...
    try {
      ...
      for (const [symbol, value] of [
        [STREAM_STANDALONE_ID_SYMBOL, 'stnd_01ARZ3NDEKTSV4RRFFQ69G5FAV'],
        [
          STREAM_STANDALONE_ENCRYPTION_SYMBOL,
          JSON.stringify({
            v: 1,
      ...
      expect(descriptorText).toContain('StandaloneWritableStream');
      expect(descriptorText).toContain('stnd_01ARZ3NDEKTSV4RRFFQ69G5FAV');
      expect(() =>
        hydrateData(serialized, {
          WritableStream: () => new WritableStream(),
        } as any)
      ).toThrow(/StandaloneWritableStream|Unknown type/);
      ...
      expect(close).toHaveBeenCalledWith('stnd_01ARZ3NDEKTSV4RRFFQ69G5FAV');
