...
export interface Hook<T = any> extends AsyncIterable<T>, Thenable<T> {
  ...
  /**
   * Returns a promise that resolves with `{ runId }` identifying the
   * conflicting run if another active hook already owns this hook's
   * token, or `null` once the hook has been registered and is ready to
   * receive payloads.
   *
   ...
   *
   * When a conflict is detected, the resolved `runId` identifies the run
   * that currently owns the token. The workflow can decide how to handle
   * the duplicate in code: return or log `conflict.runId`, or act on the
   * owner — inspect its status, await its result, or cancel it — by
   * passing `conflict.runId` to `getRun()` inside a step.
   *
