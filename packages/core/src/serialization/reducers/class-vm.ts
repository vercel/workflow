/**
 * VM-compatible reducers and revivers for custom class serialization.
 *
 * Identical to class.ts as of the pre-hardened implementation, but without
 * the `hardened.js` dependency (whose module scope imports `node:util` and
 * hosts the ambient guest-code recorder — both host-only concepts). Inside
 * the QuickJS VM the codec runs on VM-local values in the same realm, so
 * there is no cross-realm side-effect hazard to harden against; plain
 * property access is correct here. Same pattern as common-vm.ts.
 *
