...
import { hasProperty, readProperty } from '../hardened.js';
...
export function getStepFunctionReducer(): Partial<Reducers> {
  return {
    StepFunction: (value) => {
      ...
      // Invoking `__closureVarsFn` executes code inside the workflow VM, but
      // it is deliberately NOT recorded as a guest-code execution: the
      // function is generated by the SWC plugin and consists purely of
      // reads of lexically-scoped bindings (no property access, no calls),
      // so it cannot perturb observable VM state. Recording it would flag
      // every step that captures closure variables.
      const closureVars =
        closureVarsFn && typeof closureVarsFn === 'function'
          ? (closureVarsFn as () => Record<string, unknown>)()
          : undefined;
