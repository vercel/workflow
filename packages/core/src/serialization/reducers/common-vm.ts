...
export function getCommonReducers(): Partial<Reducers> {
  return {
    ...
    // HookConflictError carries an extra token (+ optional
    // conflictingRunId); mirror the host-side common.ts reducer.
    HookConflictError: (value) => {
      if (!(value instanceof Error) || value.name !== 'HookConflictError')
        return false;
    ...
    RuntimeDecryptionError: (value) => {
      if (!(value instanceof Error) || value.name !== 'RuntimeDecryptionError')
        return false;
    ...
    GlobalWritableStream: ((value: any) => {
      if (value == null) return false;
      ...
      if (
        ...
        return false;
      const globalId = value[Symbol.for('WORKFLOW_STREAM_GLOBAL_ID')];
      if (typeof globalId === 'string') {
        const encoded = value[Symbol.for('WORKFLOW_STREAM_GLOBAL_ENCRYPTION')];
...
export function getCommonRevivers(): Partial<Revivers> {
  return {
    ...
    HookConflictError: (value) => {
      // Prefer the registered HookConflictError class (see the FatalError
      // reviver above). Its constructor takes (token, conflictingRunId).
      const Cls = (globalThis as any)[
        Symbol.for('@workflow/errors//HookConflictError')
      ];
      let error: Error;
      if (typeof Cls === 'function') {
        error = new Cls(value.token, value.conflictingRunId);
      } else {
        error = new Error(value.message);
    ...
    RuntimeDecryptionError: (value) => {
      // Prefer the registered RuntimeDecryptionError class (see the
      // FatalError reviver above). Its constructor accepts
      ...
      let error: Error;
      if (typeof Cls === 'function') {
        const opts: { cause?: unknown; context?: unknown } = {};
        if ('cause' in value) opts.cause = (value as any).cause;
        if (value.context !== undefined) opts.context = value.context;
        error = new Cls(value.message, opts);
      } else {
        error = new Error(value.message);
    ...
    GlobalWritableStream: (value) => {
      ...
      stream[Symbol.for('WORKFLOW_STREAM_GLOBAL_ID')] = value.id;
      stream[Symbol.for('WORKFLOW_STREAM_GLOBAL_ENCRYPTION')] = JSON.stringify(
        value.encryption
      );
      return stream;
    },
