/**
 * VM-compatible reducer and reviver for step function references.
 *
 * Identical to step-function.ts as of the pre-hardened implementation, but
 * without the `hardened.js` dependency (host-only: `node:util` import and
 * ambient guest-code recorder). In-VM serialization runs on same-realm
 * values, so hardened introspection is unnecessary by construction. Same
 * pattern as common-vm.ts.
 *
