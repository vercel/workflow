...
/**
 * Functions this package generated and therefore knows the behavior of.
 *
 * The step-function reducer must invoke `__closureVarsFn` to read a step's
 * captured closure variables. The SWC-generated function is a sequence of
 * lexical reads and cannot perturb observable VM state, so invoking it is
 * not worth reporting — but the *property* is reachable from workflow code,
 * which can replace it with an arbitrary function. Registering the generated
 * function when the step proxy is built (host-side, in `step.ts`) makes that
 * distinction checkable instead of assumed.
 */
const trustedFunctions = new WeakSet<object>();

/** Marks a function as generated by this package. See {@link isTrustedFunction}. */
export function registerTrustedFunction<T extends object>(fn: T): T {
  trustedFunctions.add(fn);
  return fn;
}

/** Whether `fn` was registered by {@link registerTrustedFunction}. */
export function isTrustedFunction(fn: unknown): boolean {
  return typeof fn === 'function' && trustedFunctions.has(fn as object);
}
