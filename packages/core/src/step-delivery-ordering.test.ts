 ...
 *   result (`getOrHydrateStepReturnValue`). That hop count is not fixed: the
 *   first hydration pays async decrypt/deserialize, while a later replay
 *   sharing the same `stepHydrationCache` hits the memo for small primitive
 *   results and resolves in one or two hops.
 *
 ...
 * `StepHydrationCache` can be passed in, so two sequential replays can share
 * one cache exactly like the replay loop inside a single production queue
 * delivery does (see the `step-hydration-cache.ts` module docs).
 */
 ...
 * Production shares memoized primitive results across the replays of one queue
 * delivery, so the second replay of a small primitive step result never reaches
 * this spy — which is exactly the hop-count asymmetry under test.
 */
...
describe('step result delivery ordering across replays', () => {
  ...
  describe('step_completed adjacent to wait_completed', () => {
    ...
    async function buildEventLog(): Promise<Event[]> {
      ...
      // sharing the cache resolves it from the step hydration cache without
    ...
    it('delivers the wait before the step result on a later replay sharing the payload cache', async () => {
      ...
      // `StepHydrationCache` across every replay of one queue delivery.
      const sharedCache = createStepHydrationCache();
  ...
  describe('step_completed adjacent to hook_received', () => {
    ...
    it('delivers the hook payload before the step result on a later replay sharing the payload cache', async () => {
      ...
      // straight from the step hydration cache.
      const secondCtx = setupWorkflowContext(events, sharedCache);
      const { error } = await runWithDiscontinuation(
        secondCtx,
