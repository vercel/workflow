 ...
 *
 * `epoch` is the re-dispatch watchdog's count of how many times this step's
 * dispatch has been presumed lost (`stepDispatchEpoch` in
 * runtime/step-dispatch.ts). Identity scoping alone cannot recover a lost
 * dispatch: the queue holds the key for the lifetime of the message sent under
 * it, so a step whose message will never make progress needs a key the queue
 * has not seen. Epoch 0 is the unsuffixed identity key, so producers that have
 * no lost dispatch to account for (the suspension handler's parallel publish, a
 * delayed retry) keep sharing one key with the dispatch pass.
 */
export function stepDispatchIdempotencyKey(
  ...
  const identity = `${correlationId}:${fnv1a32Hex(stepName)}`;
  return epoch === 0 ? identity : `${identity}:dispatch:${epoch}`;
}
