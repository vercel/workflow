        ...
        (async () => {
          ...
          if (
            ...
            if (createResult.status === 'rejected') {
              ...
              // retried. A retryably-rejected create leaves no timestamp to
              // anchor from, but its message did go out (a queue failure above
              // is fatal), and the next replay reads the timestamp off the
              // re-ensured event.
              if (createResult.value.event?.createdAt) {
                queueItem.createdEventAt = +new Date(
                  createResult.value.event.createdAt
                );
              }
