...
import { globalSingleton, parseDurationToDate } from '@workflow/utils';
...
function doNotRetry<T extends Error>(error: T): T {
  return Object.assign(error, { fatal: true as const });
}

function normalizeStartHook(options: StartHookOptions): StartHook {
  if (options.token.length === 0) {
    throw doNotRetry(
      new WorkflowRuntimeError('hook.token must be a non-empty string.')
    );
  }
  ...
  return await waitedUntil(() => {
    return trace(spanName, async (span) => {
      ...
      if (deploymentId === currentDeploymentId) {
        ...
        if (startHook) {
          throw atomicStartContractError(
            'Cannot verify atomic start Hook support because the configured World cannot read health-check streams.'
          );
        }
        ...
        if (startHook && !probe.healthy) {
          throw new WorkflowWorldError(
            `Could not verify atomic start Hook support: ${probe.error}`,
            { code: 'TRANSPORT', cause: probe.error }
          );
        }
        const healthyProbe = probe.healthy ? probe : undefined;
        if (
          startHook &&
          healthyProbe?.capabilities?.atomicStartHook?.active !== true
        ) {
          throw atomicStartContractError(
        ...
        if (
          startHook?.tokenRetentionUntil !== undefined &&
          healthyProbe?.capabilities?.hookRetention?.active !== true
        ) {
          throw atomicStartContractError(
      ...
      const enqueueRun = () =>
        world.queue(
          ...
          queuePayload,
          {
            deploymentId,
            specVersion,
            ...(startHook ? { idempotencyKey: runId } : {}),
      ...
      if (startHook) {
        ...
        try {
          ...
          if (
            WorkflowWorldError.is(error) &&
            error.code === START_HOOK_ADMISSION_REJECTED
          ) {
            throw doNotRetry(error);
          }
