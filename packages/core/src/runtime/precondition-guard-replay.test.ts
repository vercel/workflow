...
async function runPreconditionScenario(options: {
  ...
  /**
   * Number the run's events and correlation ids by slot rather than by ULID,
   * and reject with the 409 a taken slot produces instead of the 412.
   */
  ...
  // Hooks keep their ULID ids in both modes; steps and waits count per kind
  // once the run is on slot identity.
  const hookCorrelationId = `hook_${vmUlid(+startedAt)}`;
  const syncStep0CorrelationId = options.slotIdentity
    ? `step_${slotIdBody(FIRST_SLOT)}`
    : `step_${vmUlid(+startedAt)}`;
  const waitCorrelationId = options.slotIdentity
    ? `wait_${slotIdBody(FIRST_SLOT)}`
    : `wait_${vmUlid(+startedAt)}`;
