...
export function createQuickJSSerde(
  ...
  const reducers: Record<string, (value: any) => any> = {
    ...
    GlobalWritableStream: (value) => {
      ...
      const globalId = ownSymbolString(value, 'WORKFLOW_STREAM_GLOBAL_ID');
      if (globalId) {
        ...
        if (!deploymentId || !publicKey) {
          throw new Error('Global stream writable is missing anchor metadata');
        }
        return {
          kind: 'global',
          id: globalId,
          envelope: { v: 1, s: 'dpl', d: deploymentId, k: publicKey },
        };
      }
      return false;
    },
  ...
  const revivers: Record<string, (value: any) => any> = {
    ...
    GlobalWritableStream: (value: JSValueHandle) => {
      const prototype = i.writableStreamPrototype ?? vm.null;
      const stream = call(i.objectCreate, vm.undefined, prototype);
      const id = own(value, 'id');
      const envelope = own(value, 'envelope');
      const version = envelope ? own(envelope, 'v') : undefined;
      const scheme = envelope ? own(envelope, 's') : undefined;
      const deployment = envelope ? own(envelope, 'd') : undefined;
      const publicKey = envelope ? own(envelope, 'k') : undefined;
      if (
        !id?.isString ||
        version?.toNumber() !== 1 ||
        scheme?.toString() !== 'dpl' ||
        ...
        publicKey?.dispose();
        stream.dispose();
        throw new Error('Invalid global stream writable descriptor');
      }
      define(stream, sym('WORKFLOW_STREAM_GLOBAL_ID'), id);
