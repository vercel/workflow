...
export async function executeStep(
  ...
  return trace(spanName, {}, async (span) => {
    ...
    try {
      ...
      // Any other failure (including a transient transport / 5xx world error)
      // propagates. It surfaces in the replay loop, where retryable world
      // errors are rethrown to the queue handler — earning the delivery-count
      // backoff AND the MAX_QUEUE_DELIVERIES cap. We deliberately do NOT
      // self-enqueue a fresh message here: that resets the delivery count, so a
      // persistently-failing step_started write would loop forever at a flat
      // interval and never hit the cap. Throwing on step_started is safe —
      // the step body has not run, and a write that did land server-side
      // dedupes to `skipped` on replay.
      throw err;
    }
