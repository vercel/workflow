...
async function queueStepMessage(params: {
   ...
   * (stepDispatchIdempotencyKey) so it stays mutually exclusive with the
   * node engine's dispatch of the same step, and once that dispatch is
   * presumed lost both engines suffix it with the same watchdog epoch so
   * the lost dispatch can be re-sent (see step-dispatch.ts);
  ...
  /**
   * Replay-derived watchdog state for a `dispatch` publish. Absent (or out of
   * watchdog scope) keeps the unsuffixed identity key.
   */
  ...
  await queueMessage(
    ...
    },
    {
      ...
      // and suffixed with the re-dispatch watchdog's epoch once this step's
      // dispatch is presumed lost. See stepDispatchIdempotencyKey.
      idempotencyKey:
        purpose === 'dispatch'
          ? stepDispatchIdempotencyKey(
              step.correlationId,
              step.stepId,
              dispatchState
                ? stepDispatchEpoch(dispatchState, Date.now())
                : undefined
            )
      ...
      opsPromises.push(
        (async () => {
          ...
          if (
            ...
            // Anchors the re-dispatch watchdog for this step. A retryably
            // rejected create leaves no timestamp to anchor from, but its
            // message did go out (a queue failure above is fatal), and the
            // next replay reads the timestamp off the re-ensured event.
            if (createResult.status === 'fulfilled') {
              const createdAt = createResult.value.event?.createdAt;
              if (createdAt) {
                stepCreatedAtMs.set(step.correlationId, +new Date(createdAt));
              }
            }
            return;
          }
