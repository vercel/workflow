...
import type { DeploymentStandaloneStreamEncryption } from '@workflow/world';
...
export function createReconnectingFramedStream(
  ...
  async function isVerifiedComplete(): Promise<boolean> {
    try {
      ...
      // Injected sources (standalone streams) have no run lifecycle to make a
...
/** Buffered server sink for a run-independent standalone stream. */
type StandaloneStreamWriteState = {
...
/** Buffered server sink that encrypts standalone frames against a mutable declaration. */
export class WorkflowServerStandaloneWritableStream extends WorkflowServerWritableStream {
  constructor(
    id: string,
    initialState:
      | StandaloneStreamWriteState
      | Promise<StandaloneStreamWriteState>,
    refreshState: () => Promise<StandaloneStreamWriteState>
  ) {
    ...
    const encryptFrames = async (
      chunks: Uint8Array[],
      state: StandaloneStreamWriteState
    ): Promise<Uint8Array[]> => {
    ...
    super(id, id, undefined, {
      async write(chunks) {
        const world = await getWorldLazy();
        const streams = world.standaloneStreams;
        if (!streams) {
          throw new WorkflowRuntimeError(
        ...
        for (let attempt = 0; ; attempt++) {
          ...
          try {
            const options = { encryption: state.encryption };
...
export function getExternalReducers(
  ...
  return {
    ...
    StandaloneWritableStream: (value) => {
      ...
      const standaloneId = (value as any)[STREAM_STANDALONE_ID_SYMBOL];
      if (typeof standaloneId !== 'string') return false;
      return {
        kind: 'standalone',
        id: standaloneId,
        encryption: readStandaloneStreamEncryption(
          (value as any)[STREAM_STANDALONE_ENCRYPTION_SYMBOL]
        ),
      };
    },
  ...
  return {
    ...
    StandaloneWritableStream: (value) => {
      if (!isInstanceOfPrototype(value, writableStreamPrototype.value))
        return false;
      const standaloneId = readProperty(value, STREAM_STANDALONE_ID_SYMBOL);
      if (typeof standaloneId !== 'string') return false;
      return {
        kind: 'standalone',
        id: standaloneId,
        encryption: readStandaloneStreamEncryption(
          readProperty(value, STREAM_STANDALONE_ENCRYPTION_SYMBOL)
        ),
      };
    },
    WritableStream: (value) => {
 ...
 */
function canonicalStandaloneStreamEncryption(
  encryption: import('@workflow/world').StandaloneStreamEncryption
): string {
...
function readStandaloneStreamEncryption(
  value: unknown
): import('@workflow/world').StandaloneStreamEncryption {
  ...
  try {
    return JSON.parse(
      value
    ) as import('@workflow/world').StandaloneStreamEncryption;
  } catch (cause) {
    throw new SerializationError(
      'Standalone stream encryption declaration is invalid.',
      {
        cause,
      }
    );
...
function requireDeploymentEncryption(
  ...
  if (
    ...
    throw new WorkflowRuntimeError(
      `Unsupported standalone stream encryption scheme: ${encryption.s}`
    );
  }
  return encryption as DeploymentStandaloneStreamEncryption;
}

async function getStandaloneWritableEncryptionKey(
  id: string,
  encryptionInput: import('@workflow/world').StandaloneStreamEncryption
): Promise<StandaloneStreamWriteState> {
  const encryption = requireDeploymentEncryption(encryptionInput);
  const world = await getWorldLazy();
  ...
  return {
    ...
    ReadableStream: (value) => {
      ...
      if (value.type === 'bytes') {
        ...
        // unwrap the length-prefix encryption before handing chunks to
    ...
    StandaloneWritableStream: (value) => {
      const key = getStandaloneWritableEncryptionKey(
        value.id,
        value.encryption
      );
      ...
      const refresh = async () => {
        const world = await getWorldLazy();
        const info = await world.standaloneStreams?.getInfo(value.id);
        if (!info?.encryption) {
          throw new WorkflowRuntimeError(
            `Standalone stream ${value.id} is missing its encryption`
          );
        }
        return getStandaloneWritableEncryptionKey(value.id, info.encryption);
      };
      ...
      Object.defineProperties(serialize.writable, {
        [STREAM_STANDALONE_ID_SYMBOL]: { value: value.id },
        [STREAM_STANDALONE_ENCRYPTION_SYMBOL]: {
          value: canonicalStandaloneStreamEncryption(value.encryption),
        },
      });
  ...
  return {
    ...
    StandaloneWritableStream: (value) =>
      Object.create(global.WritableStream.prototype, {
        [STREAM_STANDALONE_ID_SYMBOL]: { value: value.id, writable: false },
        [STREAM_STANDALONE_ENCRYPTION_SYMBOL]: {
          value: canonicalStandaloneStreamEncryption(value.encryption),
          writable: false,
        },
