...
function makeMockWorld(): World {
  ...
  const writeToStream = vi.fn(
    async (name: string, _runId: string, chunk: string | Uint8Array) => {
  ...
  return {
    ...
    writeToStreamMulti: vi.fn(
      async (name: string, runId: string, chunks: (string | Uint8Array)[]) => {
        for (const chunk of chunks) {
          await writeToStream(name, runId, chunk);
        }
      }
    ),
    readFromStream: vi.fn(async (name: string) => {
    ...
    closeStream: vi.fn(async (name: string, _runId: string) => {
...
describe('byte-stream framing end-to-end through dehydrate/hydrate', () => {
  ...
  it('round-trips a framed byte stream: producer writes framed, consumer unframes', async () => {
    ...
    const world = (await import('./runtime/world.js')).getWorld();
    const wireStream = await world.readFromStream(name);
  ...
  it('framer output written chunk-by-chunk to a world stream unframes back to the original chunks', async () => {
    ...
    for (const b of wireBytes) {
      await world.writeToStream('strm_known', 'wrun_test', b);
    }

    // Now read back via wire stream + unframer — should produce original chunks.
    const wireStream = await world.readFromStream('strm_known');
