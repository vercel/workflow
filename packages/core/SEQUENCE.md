...
`Sequence<T>` is an experimental immutable append-only value for step-owned
history. New additions and declarative reconstruction recipes are committed
...
`Sequence.from(compactedMessages)` inside a step.
...
A Sequence reference identifies an accepted producing step output, a result-local
...
Workflow code can carry a committed Sequence and call `take()`. Creating,
...
- Full observability UI materialization is not implemented. Ordinary sibling
  output fields hydrate normally; Sequence descriptors remain inspectable without
  reading content.
