...
// The installed quickjs-wasi version identifies the exact WASM build the
// base64 blobs below came from. Persisted into VM snapshot metadata and
// compared on restore: the QJSS heap-image header is identical across
// builds, so without this pin a snapshot taken by one build would restore
// under another and execute as undefined behavior in the interpreter
// (real hazard during a rolling deploy that bumps quickjs-wasi).
const quickjsWasiVersion = JSON.parse(
  readFileSync(require_.resolve('quickjs-wasi/package.json'), 'utf8')
).version;
...
output += `/**
 * The quickjs-wasi package version these embedded binaries came from.
 * Persisted into VM snapshot metadata (engineVersion) and compared on
 * restore — a heap image only restores under the exact WASM build that
 * produced it (the QJSS header does not distinguish builds).
 */
export const quickjsWasiVersion = ${JSON.stringify(quickjsWasiVersion)};\n\n`;
