...
`History<T>` is an experimental immutable append-only value for step-owned
...
`History.from(compactedMessages)` inside a step.
...
A History reference identifies an accepted producing step output, a result-local
...
Workflow code can carry a committed History and call `take()`. Creating,
...
- Full observability UI materialization is not implemented. Ordinary sibling
  output fields hydrate normally; History descriptors remain inspectable without
  reading content.
