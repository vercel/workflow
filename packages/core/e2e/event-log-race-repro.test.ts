 ...
 * production runner whose single-writer logs failed to replay. `hook-sleep` is retained
...
function validateDagRunnerReturn(value: unknown): { error?: string } {
  ...
  for (const entry of ledger) {
    if (!isRecord(entry) || typeof entry.node !== 'number') {
      return { error: 'Ledger entry was not a node record.' };
    }
    if (nodes.has(entry.node)) {
      return { error: `Node ${entry.node} settled twice.` };
    }
    nodes.add(entry.node);
    if (entry.worker && entry.callback === 'none') {
      return {
        error: `Worker node ${entry.node} recorded no callback outcome.`,
      };
    }
    if (!entry.worker && entry.callback !== 'none') {
      return {
        error: `Non-worker node ${entry.node} recorded a callback outcome.`,
      };
    }
  }
