...
fn step_mode(input: PathBuf) {
    ...
    test_fixture(
        Default::default(),
        &|_| visit_mut_pass(StepTransform::new(TransformMode::Step, input.file_name().unwrap().to_string_lossy().to_string(), None)),
        &input,
        &step_output,
        FixtureTestConfig {
            module: Some(true),
...
fn workflow_mode(input: PathBuf) {
    ...
    test_fixture(
        Default::default(),
        &|_| visit_mut_pass(StepTransform::new(TransformMode::Workflow, input.file_name().unwrap().to_string_lossy().to_string(), None)),
        &input,
        &workflow_output,
        FixtureTestConfig {
            module: Some(true),
...
fn client_mode(input: PathBuf) {
    ...
    test_fixture(
        Default::default(),
        &|_| visit_mut_pass(StepTransform::new(TransformMode::Client, input.file_name().unwrap().to_string_lossy().to_string(), None)),
        &input,
        &client_output,
        FixtureTestConfig {
            module: Some(true),
