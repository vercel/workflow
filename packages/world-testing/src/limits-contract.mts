...
function concurrencyDefinition(max: number): LimitDefinition {
  return {
    concurrency: { max },
  };
}

function rateDefinition(count: number, periodMs: number): LimitDefinition {
  return {
    rate: { count, periodMs },
  };
}
  ...
  describe(name, () => {
    ...
    it('keeps rate capacity consumed until the window expires', async () => {
      ...
      try {
        ...
        const definition = rateDefinition(1, periodMs);
        const first = expectAcquired(
          await harness.limits.acquire(
            acquireRequest({
              owner: ownerA,
    ...
    it('returns a combined blocked reason when both limits are saturated', async () => {
      ...
      try {
        ...
        const definition = mixedDefinition(1, 1, periodMs);
        const first = expectAcquired(
          await harness.limits.acquire(
            acquireRequest({
              owner: ownerA,
    ...
    it('extends lease expiry when heartbeated', async () => {
      ...
      try {
        ...
        const definition = concurrencyDefinition(1);
        const first = expectAcquired(
          await harness.limits.acquire(
            acquireRequest({
              owner: ownerA,
    ...
    it('prunes terminal holders during release before promoting the next waiter', async () => {
      ...
      try {
        ...
        const definition = concurrencyDefinition(3);
