 ...
 * `writeToStream`/`closeStream` API, which has no guard against writing to
 * an already-closed stream. `readFromStream()` ignores those rows;
 * `getStreamChunks()` and `getStreamInfo()` read the same table and must
 * ignore them too.
 */
describe('Postgres stream reads after a retried terminal write', () => {
  ...
  async function writeStreamWithRetriedClose(runId: string, name: string) {
    for (const text of ['a', 'b', 'c', 'd', 'e']) {
      await streamer.writeToStream(name, runId, text);
    }
    ...
    await streamer.writeToStream(name, runId, 'e');
    await streamer.closeStream(name, runId);
  }

  test('getStreamChunks() excludes rows written after the first EOF', async () => {
    const runId = `run_${randomUUID()}`;
    const name = `stream_${randomUUID()}`;
    await writeStreamWithRetriedClose(runId, name);

    const result = await streamer.getStreamChunks(name, runId);
  ...
  test('getStreamChunks() paginates only up to the first EOF, not past it', async () => {
    const runId = `run_${randomUUID()}`;
    const name = `stream_${randomUUID()}`;
    await writeStreamWithRetriedClose(runId, name);

    const page = await streamer.getStreamChunks(name, runId, { limit: 3 });
    ...
    const rest = await streamer.getStreamChunks(name, runId, {
  ...
  test('getStreamInfo() does not count rows written after the first EOF', async () => {
    const runId = `run_${randomUUID()}`;
    const name = `stream_${randomUUID()}`;
    await writeStreamWithRetriedClose(runId, name);

    const info = await streamer.getStreamInfo(name, runId);
