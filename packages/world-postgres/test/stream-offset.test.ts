...
/**
 * `readFromStream(name, startIndex)` skips `startIndex` chunks before it
...
describe('Postgres stream offsets', () => {
  ...
  test('a repeated notification for a skipped chunk does not consume the offset twice', async () => {
    ...
    const stream = await streamer.readFromStream(name, 2);
    const reader = stream.getReader();
    try {
      await streamer.writeToStream(name, runId, 'first');
      ...
      await streamer.writeToStream(name, runId, 'second');
      await streamer.writeToStream(name, runId, 'third');
