...
/**
 * A run row and its run_created event have to become visible together. A
 * creation that fails after the row is in place leaves a run with no first
 * event, which no replay can reconstruct, and a concurrent reader must never
 * observe that intermediate state. Both creation sites are covered: an
 * explicit run_created, and the resilient run_started that recreates the run
...
describe('atomic run creation', () => {
  ...
  const storedEventTypes = async (runId: string) => {
    const stored = await observer.query(
      `SELECT type FROM workflow.workflow_events
  ...
  test.each([
    'run_created',
    'run_started',
  ] as const)('%s remains invisible while its event insert is blocked', async (eventType) => {
    const runId = `wrun_${ulid()}`;
    const blocker = await observer.connect();
    await blocker.query('BEGIN');
    await blocker.query(
      'LOCK TABLE workflow.workflow_events IN ACCESS EXCLUSIVE MODE'
    );
      ...
      await expect
        .poll(async () => {
          const result = await observer.query(
            ...
            [applicationName, '%workflow_events%']
          );
    ...
    const expected =
      eventType === 'run_started'
        ...
        : ['run_created'];
    expect(await storedEventTypes(runId)).toEqual(expected);
  });

  test.each([
    'run_created',
    'run_started',
  ] as const)('%s rolls back its run if the first event fails', async (eventType) => {
    const runId = `wrun_${ulid()}`;
  ...
  test('a duplicate creator preserves the existing first event', async () => {
    ...
    expect(await storedEventTypes(runId)).toEqual([
      'run_created',
      'run_started',
    ]);
  });
