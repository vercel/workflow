...
describe('Postgres queue HTTP deadlines (integration)', () => {
  ...
  test.each([
    ...
    const queue = createQueue({ connectionString, queueConcurrency: 1 }, pool);
    try {
  ...
  test('a same-run wake can release an inline step before its delivery finishes', async () => {
    ...
    const queue = createQueue({ connectionString, queueConcurrency: 2 }, pool);
