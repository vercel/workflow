...
That endpoint is not authenticated: the queue HTTP handler is inherited from `@workflow/world-local` and checks the `x-vqs-*` header shape, the queue-name prefix, and the payload schema, but never the caller — so restricting the route is left to the deployment. See [Security](./README.md#security).
