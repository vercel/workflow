...
`world.close()` first stops Graphile Worker from claiming new jobs, then waits for active jobs before closing the streamer and any internally owned pool. Repeated or concurrent calls share the same shutdown.
