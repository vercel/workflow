...
describe('Storage', () => {
  ...
  describe('step_deferred event handling', () => {
    let testRunId: string;
    ...
    it('should roll back the first blocked attempt without recording an error', async () => {
      await createStep(storage, testRunId, {
        stepId: 'step_deferred_1',
        stepName: 'test-step',
        input: new Uint8Array(),
      });
      await updateStep(storage, testRunId, 'step_deferred_1', 'step_started');

      const retryAfter = new Date(Date.now() + 5_000);
      const result = await storage.events.create(testRunId, {
        eventType: 'step_deferred',
        correlationId: 'step_deferred_1',
        eventData: {
          retryAfter,
        },
      });
    ...
    it('should preserve the original startedAt after a prior real attempt', async () => {
      await createStep(storage, testRunId, {
        stepId: 'step_deferred_2',
        stepName: 'test-step',
        input: new Uint8Array(),
      });
      ...
      await storage.events.create(testRunId, {
        eventType: 'step_retrying',
        correlationId: 'step_deferred_2',
        eventData: { error: 'Temporary failure' },
      });
      await updateStep(storage, testRunId, 'step_deferred_2', 'step_started');
