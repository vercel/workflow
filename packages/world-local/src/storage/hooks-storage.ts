...
/**
 * Path to the durable token-claim file for a hook token. On world-local
 * this doubles as the token uniqueness constraint written on hook
 * creation; it records the owning `hookId`, so a token lookup can jump
 * straight to the hook entity without scanning.
 */
...
/**
 * Path to the dispose lock written before a `hook_disposed` event is
 * appended (see the disposal handler in events-storage.ts). Its
 * presence means the hook is being (or has been) torn down.
 */
function hookDisposeLockPath(
  basedir: string,
  hookId: string,
  tag: string | undefined
): string {
  const lockName = tag ? `${hookId}.disposed.${tag}` : `${hookId}.disposed`;
  return path.join(basedir, '.locks', 'hooks', lockName);
}
 ...
 * writes the lock, then removes the constraint file and hook entity, and
 * only then appends the event. Rebuilding the caches from the log in that
 * window would resurrect a hook that is being torn down.
 */
async function isHookDisposalCommitted(
  basedir: string,
  hookId: string,
  tag: string | undefined
): Promise<boolean> {
  try {
    await stat(hookDisposeLockPath(basedir, hookId, tag));
    return true;
  } catch (error) {
...
/**
 * Release the token claim/constraint for a hook, but only if it still
 * points at this hook — a claimant may already hold a fresh claim for
 * the token, which an unconditional delete would destroy.
 */
async function releaseHookTokenClaimIfOwnedBy(
  basedir: string,
  token: string,
  runId: string,
  hookId: string
  ...
  let claim: { hookId?: string; runId?: string } | null = null;
  try {
    claim = await readJSON(
      claimPath,
      z.object({ hookId: z.string().optional(), runId: z.string().optional() })
    );
  } catch (error) {
    if (error instanceof SyntaxError || error instanceof z.ZodError) return;
    throw error;
  }
  if (!claim || (claim.hookId === hookId && claim.runId === runId)) {
...
function hookFromCreatedEvent(event: Event & HookCreatedEvent): Hook {
  const { token, metadata, isWebhook } = event.eventData;
  return {
    runId: event.runId,
