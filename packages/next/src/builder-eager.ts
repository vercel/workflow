...
export async function getNextBuilderEager(
  ...
  class NextBuilder extends BaseBuilderClass {
    ...
    async build() {
      ...
      await this.buildInvocationRoute({ workflowGeneratedDir });
    ...
    private async buildInvocationRoute({
      workflowGeneratedDir,
    }: {
      workflowGeneratedDir: string;
    }): Promise<void> {
      const invocationRouteDir = join(workflowGeneratedDir, 'invoke');
      await mkdir(invocationRouteDir, { recursive: true });
      // Reuse the execution implementation without assigning a queue trigger
      // to the HTTP entry point. The World authenticates direct requests.
      await writeFileIfChanged(
        join(invocationRouteDir, 'route.js'),
        `// Generated by Workflow.\nexport { POST } from '../flow/route.js';\n`
      );
    }
