...
// Legacy directory name for per-file step source copies. Retained so boot
// cleanup can remove stale copies left behind by older versions that used
// the copy mechanism. No longer generated.
const LEGACY_DEFERRED_STEP_COPY_DIR_NAME = '__workflow_step_files__';
  ...
  class NextDeferredBuilder extends BaseBuilderClass {
    ...
    private async cleanupGeneratedArtifactsOnBoot(
      ...
      const staleArtifactPaths = [
        ...
        // Remove stale per-file step copies generated by older versions of
        // @workflow/next. Direct imports replaced the copy mechanism.
        join(stepRouteDir, LEGACY_DEFERRED_STEP_COPY_DIR_NAME),
